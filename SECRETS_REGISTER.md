...
|---|---|---|---|
| RESEND_API_KEY | `/etc/marketsquare/resend.watch.conf` (0640 root:msdeploy, 74 B, mtime 2026-08-05) | the daily-watch RED-alert send — the ONLY thing that wakes David when the site is down | **DEAD — day 3. RE-PROBED 2026-08-28 04:39 UTC from the box: `GET https://api.resend.com/domains` returns HTTP 400.** *(The 26 Aug reading was `401 validation_error`; the newer measurement supersedes it — both mean refused, and the register keeps the probe's number, not the remembered one.)* File untouched: 74 B, `-rw-r----- root:msdeploy`, mtime still `Aug 5 06:26`. Orphaned by the 22–23 Aug rotation, which deleted both old keys. DAVID (root on the box). DW-076 · DAVID_QUEUE D3. **CTO DECISION 2026-08-28 (RUL-037): the replacement must be a NEWLY MINTED, DEDICATED Resend key — NOT a copy of the app's `RESEND_API_KEY` out of `secrets.env`.** Copying the app key re-creates the exact coupling that just failed: the next app-key rotation would silently orphan the outage alarm a second time. An out-of-band alert credential that shares its lifecycle with the thing it watches is not out-of-band. Cost of the separation: zero, and one minute in the Resend dashboard |
