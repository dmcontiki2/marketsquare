...
- **Deploy queue cleared.** `bea_main.py:3862` `aa_buy_pack` → 410 Gone (AI-use packs retired; coach stays deliver-then-charge 1T per use after first free). ms.js AI-uses copy purged at 667/885/9617/9632 (payment-return message, pack modal → wallet top-up framing, coach email/wallet copy). `marketsquare.html:1806` Free tier card "3 free AI coaching sessions" → "Free in-app AI guidance". Seller-onboarding tier cards brought to tier canon: retired $5/$15 structure removed — Standard $12/R216/10 listings (key `starter`→`standard`), International→Professional $20/R360/25 listings (key `premium`→`professional`), sobSelTier list updated. PLANS-page Tuppence chips were already shipped by the interim 6 June v154 deploy — not duplicated.
- **Travel + Tour_Guides trust signals seeded (David-confirmed, one-click).** `_CATEGORY_SIGNALS["Travel"]` (ASATA 10 · IATA 10 · CIPC 6 · 3–7yrs 5 · 7+yrs +5 · client insurance 4 · bonding 5 = 45) and `["Tour_Guides"]` (provincial registrar badge 12 · CATHSSETA 8 · first aid 6 · 3–7yrs 5 · 7+yrs +5 · 3+ languages 4 · site specialist 5 = 45), both on the universal base 40 like every category. The travel_agency + tour_guide outreach templates are no longer blocked on signals.
- **Verify/deploy.** ast/py_compile clean local + BEA venv (staged `main.py.new` compile-before-swap); whole-file diff vs freshly-pulled server `main.py` = exactly the 7 hook regions; server backups `*-20260606-s129` (main.py, ms.js, index.html, marketsquare.db); scp ×5 sha256-matched; BEA restarted active `/health` 1.3.1 (local + public); `smoke_test.py --local` ALL OK pre + post; CF purged `{"purged":true}`; live: index serves `ms.js?v=155`, served ms.js carries fspark ×6, `/launch/redeem`→503 (gated), `/advert-agent/buy-pack`→410, demo 302 intact; `fea_integrity_check.py --update-baseline` (index 376544B, ms.js v155 729429B). Session ran concurrently with the interim v154 chat deploy — drift detected mid-session and reconciled (no clobber; their work preserved, PLANS edit skipped).
- **Cost model impact:** monthly Tuppence allocations (6/10/20/50, badge ×1.2 → 8/12/24/60) are now CODE, env-gated OFF — when enabled at launch, Tuppence issuance per paid seller = price÷2 monthly (+20% for Founders holders), per Canon Addendum №1 rev 2; the 5T→40-uses AI pack SKU is retired (coach = 1T per use after first free). No new AI calls, infra, or concurrency this session.
