...
**Map auto-alignment (`ms.js` v140->v141).** Five surgical edits (Python str-replace driver, never Edit/Write; `node --check` clean; diff = exactly the intended hunks): (1) `selectCity(id,name)` -> `selectCity(id,name,lat,lng)` captures the coords the `/geo/cities` response already returns; (2) the live-path city-option `onclick` passes `c.lat,c.lng`; (3) `renderMap()` now centres an empty-listings city on `activeCity.lat/lng` (the hardcoded dict remains only as a final fallback) — so ANY seeded city auto-centres, fixing the long-standing Cape Town/Joburg->Pretoria mis-centre; (4) `selectCity`/`selectDemoCity` re-render the map on city switch when in map view; (5) `DEMO_COUNTRY_CITIES` extended with all Wave 1/2 cities + coords and `selectDemoCity` generalised to derive country/flag/coords from it. Backed up server `ms.js`; deployed `static/ms.js`; bumped `index.html` `ms.js?v=140->141`; Cloudflare purged; served-through-CF verified; **smoke all-green** (--local).

**Verification.** In-app visual click-test not possible this session (Chrome extension offline; sandbox headless-Chromium download blocked by the network allowlist). Verified end-to-end at the data + code + deploy layers instead: the exact picker click-path (`/geo/countries` -> `/geo/regions?country=` -> `/geo/cities?region_id=`) resolves with correct coords for New York, Chicago, London, Manchester, Sydney, Cape Town, Johannesburg, Port Elizabeth; served `ms.js` carries all five edits; smoke green. In-app screenshot walkthrough pending Chrome reconnection.

**Gaps filed (BACKLOG).** (a) Wave 3 AU cities not seeded (out of Wave 1/2 scope). (b) International cities have no suburb hierarchy (suburb panel shows "All suburbs" only; free-tier suburb gating stays ZA-only). (c) Admin seller-onboarding region/city picker is still hardcoded `country=ZA`, so non-ZA prospects can't yet be assigned a `geo_city_id` through the admin UI — needs a country selector feeding region->city (the magic link carries the city name, but geo resolution for non-ZA is the gap).

**Cost model impact:** none — geo data + display-centring only; no change to AI volume, pricing, concurrency, or email/launch cadence. Wave 1/2 cities are now selectable/staged for testing, but no outreach has fired and launch cadence is unchanged (still gated on patent + whitepaper).
