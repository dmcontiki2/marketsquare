...
## Filter into the app — increment 1: global Trust ≥ filter (FEA, live) — 2026-06-03

David-directed: begin implementing the Filter design into the live buyer app, **UI-first** (the Step 1 server-side backend stays parked). First testable increment = a **global Trust ≥ filter** — the marketplace's core signal, not previously filterable in browse.

**Shipped (`ms.js`, client-side):** module-level `trustMin` + `setTrustMin()`; `renderFilterBar()` now renders a **Trust ≥ selector** (Any/60/70/80/90) on every category **and** on "All" (the bar was empty on All before); `renderGrid()` drops listings below `trustMin` (placeholders exempt); `renderActiveFilterTags()` shows a "★ Trust ≥ N" tag. Applied via the str.replace driver on the server's in-sync `ms.js` copy (node --check + diff-gated; 4 surgical edits), never Edit/Write.

**Correct at current volume:** client-side filtering over the loaded set is complete while a city's listings are ≤ the fetch size (true today at 65); the server-side version is Step 1 (parked, `STEP1_BUILD_SPEC.html`) for when cities grow large.

**Deploy + verify:** server backups `*.bak-20260603-trust`; deployed `static/ms.js` + index cache-bust v133→v134; Cloudflare purged (200); local re-synced (sha == server). node --check clean; **smoke 39/39**. Live through CF: index serves v134, served `ms.js` contains `setTrustMin` + the trust selector + the renderGrid trust line, node-valid. Browser click-test deferred to David (Chrome extension offline this session).

**Next increments** (per `FILTER_APP_MOCKUP.html` / `GLOBAL_MARKETPLACE_DESIGN.html`): Save-as-Wishlist (wire to the existing wishlist), then scope (Local/Nearby/Global) + currency — the latter two ride on the Step 1 backend. **Cost model impact:** none (client-side display/filter).
