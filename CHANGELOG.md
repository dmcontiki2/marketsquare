...
## 03 Aug 2026 — SCOREBOARD-1: silent scoreboard agent built + FLIP-DRILL-1 runbook (attended, David's ruling)
- **Why:** David's durability review ("How could we improve towards Durability and Independence?"). His ruling: rehearse the flip (fire drill) and build his scoreboard concept — nightly probes of every AI lane per capability, a rolling 3-month ranking (uptime · latency · cost) that first RECOMMENDS and, once trusted, DETERMINES flips. Design split inherited: breaker = fast signals, scoreboard = slow signals; QUALITY IS A GATE, NOT A WEIGHT.
- **Change:** new `ai_scoreboard.py` — probe rounds via the seam's direct probe mode (`complete(probe=True)`, so the breaker sees every probe through its own record hook — one truth, two consumers); history in `ai_scoreboard_probes` (primary DB → rides the backup lanes); rolling ranking (availability band → cost/typical-call → p95 latency; golden-set gate blocks ungated lanes — openai stays GATED until its server-key golden run, RG-0016) → `ai_scoreboard.json` + CLI `--probe/--report/--json`. Nightly wiring appended to `bea_main.py` (startup task 03:33 SAST, import-guarded, exception-walled — can never hurt the app). `deploy_manifest.txt` ships the agent. Guard suite `test_ai_scoreboard.py` (7/7 green on this machine) wired into `predeploy_check.py` (danger token `scoreboard-guards`).
- **Spend gate:** OFF by default — new column `launch_switches.scoreboard_enabled`; `enable_scoreboard.bat` (David's click, well under 1 US cent/night, runs one attended round + prints the first table) / `disable_scoreboard.bat`. Order: deploy first, then enable.
- **Also:** `FIRE_DRILL_RUNBOOK.html` — FLIP-DRILL-1: Drill A (one-hour live flip to the Scaleway lane; abort = a rehearsed result, not a failure) + Drill B (`AI_DRILL_BAN=anthropic` ban simulation per P2 design §8). Durability Map re-surveyed to the 3 Aug position (Rung 2 closed, ladder ~85%).
- **Live:** nothing yet — rides the next deploy-ref publish; probes stay OFF until enable_scoreboard.bat.
...
- **Applied** via Python str.replace driver (anchor asserted unique==1, never Edit/Write; LF preserved). Backup `activate_autodeploy.bat.bak-rg0015-20260802-112725`.
- **Verified:** `regression_ledger.py` BEFORE = 1 REGRESSED (RG-0015), exit 1 → AFTER = 22 entries · 20 holding · 0 REGRESSED · 2 open (RG-0003/0004 known currency) · **exit 0 GREEN**.
- **Loop health:** health ok v1.3.1 · smoke 40/40 · AI spend \$0.00/\$100 · subscriptions 35 UP / 15 HELD / 1 PLANNED, 0 issues · cron-parity MATCH (cron 39/39 on-box vs live 40/40 — server smoke_test.py one test behind repo, benign) · FEA baseline refreshed to clear the 01-Aug deploy-drift alert. Late run (+~5h vs 06:30). Session counter unchanged (150) — LOW auto-ship, not attended.

## 01 Aug 2026 — EULA v1.11: affiliate-disclosure clause (TP-FLIGHTS-1c, attended, David: "operate on the safe side of caution")
- **Why:** v1.10 SS6.1 stated "no ... commissions ... The Platform earns from Introduction fees, subscriptions, and paid AI features" — enumerated income list would become FALSE the day Travelpayouts commission flows. Found and fixed BEFORE the first cent (flags still dark; zero exposure window).
- **Change:** new **SS6.1A Third-Party Referral (Affiliate) Links** (distinct-from-Introductions per SS2.6 · commission paid by provider never by user · Platform not the seller/party · indicative-prices caveat) + SS6.1 sentence amended to disclose referral commissions "payable by you"-scoped. Version 1.10 -> 1.11, dated 1 Aug 2026.
- **Three synced copies edited together** (the modal-drift trap avoided): eula_clean.html (source), terms.html (/terms), ms.js (acceptance modal, JS-string-safe ASCII, node --check clean). Backups `*.bak-20260801-eula111`.
- **Shipped from the cloud via SEC-2** /admin/deploy-file (terms.html + ms.js, sha256-verified) + CF purge; live /terms shows v1.11. NB: version bump means users re-accept on next gate pass — intended.
- **Still open (OPEN_LOOPS [D]):** counsel ratification of SS6.1A wording; accountant tax/VAT confirmation; in-app disclosure line ships with the planner's click-out UI. Flag flip stays blocked on these.

## 01 Aug 2026 — TP-FLIGHTS-1: Travelpayouts replaces the dead Amadeus slot (attended, David)
- **Why:** Amadeus decommissioned its self-service API portal 17 Jul 2026 (keys dead, no new registrations) — the reserved `AMADEUS_API_KEY` slot could never be filled. Google is OUT (silent ~$360 bill, David's ruling). Travelpayouts/Aviasales chosen as the flights pre-information lane: introduction-model compatible (traveler books off-platform, commission IN, no money through the till), R0 cost, no contract.
- **Account:** Travelpayouts partner ID 758984, project "Trustsquare" connected to the Aviasales program (blockers were the unread activation email + missing Project — both cleared this session). Flight DATA API available without request. Dry run in native ZAR: JNB-CPT R2,267 / JNB-DUR R1,348 / JNB-WDH R5,080 / JNB-LHR R11,417 returned; CPT-GRJ empty `{}` (thin-route cache gap — adapter must treat empty as no-fare fallback, not error).
- **Change (bea_main.py, one row):** DATA & PARTNERS panel tuple `("amadeus", …, "AMADEUS_API_KEY", "data_flights")` → `("travelpayouts", …, "TRAVELPAYOUTS_TOKEN", "data_flights")`. Presence-only like its siblings (per the section's standing rule: never live-probe partners from a panel refresh). Flag `data_flights` stays DARK. Backup `bea_main.py.bak-20260801-tp`, py_compile clean.
- **Server env:** `TRAVELPAYOUTS_TOKEN` appended to `/var/www/marketsquare/.env` (backed up first). Token NEVER committed to files/git.
- **SHIPPED LIVE (same day, ~18:40):** via new `add_travelpayouts_key.bat` (drift-guard md5 → token drop-in `travelpayouts.conf` per add_resend_key pattern → scp + server py_compile gate → backup `main.py.bak-tpflights-20260801` → atomic swap → restart): active, /health ok v1.3.1, TRAVELPAYOUTS_TOKEN confirmed in service env. v1 of the bat failed safe (drift guard aborted on empty hash): cmd `for /f ('ssh -o ConnectTimeout=15 …')` eats the `=` — never pass `-o Key=Value` inside for /f; v2 writes to %TEMP% + `set /p`. Rollback: `cp main.py.bak-tpflights-20260801 main.py && systemctl restart marketsquare`.
- **TP-FLIGHTS-1b (same evening):** three forward lanes added to the DATA & PARTNERS panel so nothing waits invisibly — `duffel` (warm standby, reserved DUFFEL_ACCESS_TOKEN, grey), `aerodatabox` (deferred by ruling, reserved AERODATABOX_KEY, grey), `tp_tours` (GYG · Viator · Welcome Pickups · Booking.com, amber "awaiting project review", no extra key — rides TRAVELPAYOUTS_TOKEN). Presence-only, zero spend. Backup `bea_main.py.bak-20260801-tp2`; shipped via retargeted add_travelpayouts_key.bat (EXPECT_MD5 → 2016cf…, rollback bak-tpflights2).
- **Doctrine (CLAUDE.md):** supplier-fallback recorded — app reads only our own fare cache; suppliers are swappable adapters (Duffel search $0.005 capped = warm standby; signed-agency fare cards = irrevocable floor).

## 01 Aug 2026 — INFRA-PANEL-2: Infrastructure card no longer sticks on "Loading checks…" (root-cause fix)
- **Fault (recurring since INFRA-PANEL-1, 22 Jul):** the dashboard's Infrastructure card sat silently on "Loading checks…" with only a small grey "Admin session expired — reload + PIN." note — every fresh visit and every server-side JWT expiry reproduced it. Reload+PIN was the manual workaround being repeated, not a fix.
- **Root cause:** `infraLoad()` fires at page parse, before PIN sign-in (or with an expired token) → 401 → the handler wrote a note into `#infra-out` but left the static "Loading checks…" placeholder in `#infra-rows` untouched, and NOTHING re-ran the load after successful sign-in except the 5-minute poll. Structural: auth success had no signal the token-gated cards could react to.
- **Fix (dashboard.server.html, INFRA-PANEL-2):** (1) `hideGate()` now dispatches a `ms-admin-auth` event on every successful auth path (stored-token verify, PIN login, PIN change); the infra card listens and reloads instantly on sign-in. (2) 401 renders a LOUD amber row state — "Checks paused — admin session signed out… resumes automatically after PIN sign-in" + Retry — never leaves "Loading checks…". (3) Network failure with no cached data renders a red "Checks could not load" + Retry (`infraFailLoud`). (4) Token-aware bootstrap: signed-out → paused state immediately, zero doomed 401 round-trips; the 5-min poll also skips while signed out.
- **Verified:** `node --check` clean on all 10 inline script blocks; 9-scenario state-machine simulation (fresh visit no-token / poll-skip / PIN sign-in / mid-session expiry / re-auth / network failure) all green. Local file updated + `.bak-20260801-infrapanel2`; ships to the live dashboard with the next dashboard push (NB: `deploy_bit_monitoring.bat` pulls server→local first and would clobber this — push dashboard.server.html before/without that pull).
- **ADDENDUM (same day, ~09:54):** the clobber predicted above HAPPENED — David ran `deploy_bit_monitoring.bat`; its step-1 server→local pull overwrote the local fix and pushed the un-fixed dashboard back up. Closed at class level the same way the BIT panel survives the pull: new **`apply_infra_panel2.py`** (idempotent, 5 anchors asserted unique, CRLF-preserving, no-op when `ms-admin-auth` already present) wired into `deploy_bit_monitoring.bat` as step **[2c/6]** after the BIT + AI-provider patchers — every future bit-monitoring deploy re-applies the fix automatically, pull or no pull. Proven on the freshly pulled server copy: applied → idempotent re-run no-op → node --check 10/10 → state-machine simulation 9/9 green. Live from the next bit-monitoring run.

## 01 Aug 2026 — Daily loop: SCAN-29 shipped (ruff B904 ×2 in /admin/deploy-file)
- **SCAN-29 · LOW · DONE (auto-ship, Gate 1+2 clear).** `admin_deploy_file` (the MS_DEPLOY_KEY-gated HTTPS deploy channel, SEC-2): added `from None` to the base64-decode 400 raise (`main.py:15994`) and `from exc` to the write-failed 500 raise (`main.py:16010`) — behaviour-neutral exception chaining, clears ruff B904. Security-class, no payments/ledger/EULA/KYC.
- **Parity-block cleared first:** SCAN-29 had been HELD since 2026-07-31 because local `bea_main.py` carried an undeployed ATTENDED AI-vendor change (a whole-file scp would have dragged it live). David attended-/shipped that WIP 2026-07-31 05:00; deploy-drift is now clean (19/19 tracked files match live, local `bea_main.py` == server `main.py`), so the whole-file scp is safe.
- **Ship:** local==server md5 `27b9bb96…` pre-edit; Python str.replace driver (anchors asserted unique==1; never Edit/Write), +19B, AST clean local + server venv. Server backup `main.py.bak-scan29-20260801-063938`; scp `.new` → venv AST-check → `mv` → `chmod 644`; restart active; /health ok v1.3.1; md5 parity local==server `d13ccfdf…`; both chained lines confirmed on served copy; smoke 40/40 pre+post. CF purge n/a (backend module). Static-scan auto-ship queue now EMPTY.
- **Health of the loop:** regression ledger GREEN (exit 0, all LOCKED holding, 2 known-open currency), FEA ok, subscriptions 34 UP/1 STANDBY/15 HELD/1 PLANNED 0 issues, cron-parity MATCH, AI spend $0.00/$100. Session counter unchanged (150) — LOW auto-ship, not attended.
- Files: server `main.py` / local `bea_main.py` (+ `.bak-scan29-*`). Docs: CHANGELOG/AUDIT_PROGRESS/STATUS.

## 30 Jul 2026 — GIT-LOCK-1: the recurring `.git/index.lock` failure fixed at the class level (attended, David: "why does this keep happening")
- **Symptom (recurring across many sessions):** `git add`/`git commit` fail with *"Unable to create '.git/index.lock': File exists"*, blocking David's manual commit. Every prior session only deleted the lock and re-ran — a reminder, never a fix.
- **Root cause:** several UNCOORDINATED git writers share one repo — manual `commit.bat`, `nightly_checkpoint.bat` (Task Scheduler), the deploy auto-commit, and the daily-loop sandbox. When two overlap or one is interrupted git leaves a 0-byte `index.lock`; on the FUSE-mounted sandbox it can't be removed at all, so it persists and blocks the NEXT commit. `deploy_marketsquare.bat` got an inline stale-lock sweep (22 Jul) but the manual + checkpoint committers — exactly the paths David uses — never did.
- **Fix (class-level, self-healing):** new `git_unlock.bat` removes a stale `.git/index.lock` ONLY when no `git.exe` is running (so it can never race a live commit). Every git-writing .bat now calls it before staging: `commit.bat`, `commit_checkpoint.bat`, `nightly_checkpoint.bat`, `commit_tsl_goodstate.bat`, `deploy_eula_v19.bat`, `deploy_files.bat`, `untrack_media.bat` (`deploy_marketsquare.bat` already self-heals inline). 8/8 committers guarded.
- **Locked in the ledger:** RG-0015 asserts every git-committing .bat clears a stale lock first (calls `git_unlock.bat` OR inline `del ... index.lock`). A new unguarded committer trips it red. `regression_ledger.py` GREEN, exit 0.
- **Sandbox no longer plants:** standing rule added to CLAUDE.md — all sandbox git runs `GIT_OPTIONAL_LOCKS=0` so read-only git (status) never takes an index lock on the FUSE-shared `.git`.
- Files: `git_unlock.bat` (new), 7 committer .bats, `scripts/regression_ledger.py` (RG-0015), CLAUDE.md, CHANGELOG (+ `.bak-20260730-*` beside each edited file).

## 30 Jul 2026 — Daily loop: SCAN-28 shipped (ruff B905 in /admin/services-status) + RG-0014 LOCKED
- **SCAN-28 · LOW · DONE (auto-ship, Gate 1+2 clear).** `bea_main.py:12241` `for i, res in zip(ids, results):` in `admin_services_status` -> added explicit `strict=False` (ids/results are equal-length by construction from `asyncio.gather` -> behaviour-neutral; clears ruff B905). Admin infra-status board; no payments/ledger/EULA/KYC. Local `bea_main.py` == server `main.py` md5 `8df61152...` pre-edit; Python str.replace driver (anchor asserted unique==1; never Edit/Write), +14B, AST clean local + server venv. Server backup `main.py.bak-20260730-scan28` (809935B); scp `.new` -> venv AST-check -> `mv` -> `chmod 644`; restart active; /health v1.3.1 public+localhost; md5 parity local==server `c113e07b...`; `/admin/services-status` re-verified 401 (route live, gated); smoke 40/40 pre+post; CF purge `{purged:true}`.
- **RG-0014 promoted OPEN->LOCKED.** The Adventures-screen red ★ SUPER ADVERT ribbon guard now passes against the live build at the `?v=` the live index references (ADV-SUPER-BADGE-1 deployed; live ms.js v411). Per the entry's own instruction + the ledger machinery rule, promoted to LOCKED so it cannot rot back. `scripts/regression_ledger.py` -> 14 entries · 11 holding · 0 REGRESSED · 3 open (RG-0003/0004/0006 pre-existing structural currency defects) · exit 0. GREEN.
- Files: `bea_main.py`/server `main.py`, `scripts/regression_ledger.py` (+ `.bak-<ts>` beside each). Docs: CHANGELOG/AUDIT_PROGRESS/STATUS. Session counter unchanged (150) — LOW auto-ship, not an attended session.

## 29 Jul 2026 — ADV-SUPER-BADGE-1: Adventures screen finally shows the red ★ SUPER ADVERT ribbon (attended, David — third pass, root cause found)
- Why two earlier fixes didn't take: SUPER-1 (Session 144) put the ribbon on the shared browse `lcard` renderer + the detail pill; the 22-Jul pass put it on Local Market cards. The Adventures tab renders through its OWN `renderAdvGrid()` inline template (ms.js ~2477), which never read `l.super_example` — so both fixes were correct for the renderers they touched and invisible on the Adventures screen. Verification passed both times because it checked browse/LM/detail, never the Adventures tab.
- **Fix (ms.js):** the identical red top-left ribbon div (matching `lcard`'s byte-for-byte) inserted into `renderAdvGrid()`'s `position:relative` image wrapper, gated on `l.super_example`. One renderer serves every country — scope is ALL Adventures markets, not ZA-only.
- **RG-0013 guard honoured:** `ms.js?v=` bumped 408→409 in marketsquare.html so the fix cannot ship browser-cache-stale.
- **Ledger:** NEW **RG-0014** (OPEN) asserts the ribbon in BOTH the repo `renderAdvGrid()` AND the live-served build at the `?v=` the live index references. Promote to LOCKED when it reports READY TO LOCK after the next deploy. Ledger run BEFORE (green, 0 regressed) and AFTER (RG-0014 open-as-expected, nothing rotted).
- Files: ms.js, marketsquare.html, scripts/regression_ledger.py, CHANGELOG.md (+ timestamped .bak beside each on disk). **NOT yet deployed** — needs a frontend deploy from David's machine (deploy_marketsquare.bat or deploy_frontend_only.bat).

## 29 Jul 2026 — REGRESSION LEDGER BACK TO GREEN: 4 RED regressions closed at root (attended, David: "complete and close these 4")
- Trigger: the daily loop held the Fixer a 3rd run (SCAN-28/29 deploy-gated by a RED regression_ledger). David directed closing the 4 RED entries. Investigation found the LIVE APP + DATA already correct on all four — 3 of 4 were the GUARD rotting (looking in the wrong place / too-strict regex), 1 was a genuinely-unfixed local deploy script. **No production deploy required; fixes are repo/tooling only.**
- **RG-0002 / RG-0005 (maun/Botswana currency) — ledger model completion.** App already renders BW correctly (ms.js `ADV_COUNTRY_CURRENCY` has `BW:'P'`; `demo_stay_bw_1` country='BW'). Gap was the ledger's own market model: added `"maun":"P"` (CITY_CCY), `"maun":"BW"` (CITY_COUNTRY), `"BW":"P"` (RG-0002 ADV table). Records true facts (Maun∈Botswana, Pula='P'); NOT a weakening.
- **RG-0012 (c2c + NA tour maps) — a guard reading the wrong feed.** Invariant is satisfied live: Cape-to-Cairo supers 306/307 carry `tour='c2c'`/`country=ZA`; Namibia supers 304/305 carry `country='NA'`/`super_example=1`; both maps live (2.5MB/1.5MB); ms.js fully wired (NA un-gated, `ADV_TOUR_MAP[c2c]`, picker precedence, tour normalizer). The guard's live check read `/demo-listings` (no supers there) keyed on the demo `cat` field → corrected to read `/listings` (real feed, keyed on `category`). Also relaxed the NA-un-gated regex to tolerate the `?v=` cache-buster the 27-Jul fleet bump added. Invariant unchanged and still enforced.
- **RG-0013 (deploy_frontend_nops.bat) — real script fix.** The no-PowerShell quick-deploy scp'd ms.js but never bumped `?v=`, so its deploys landed under a browser-cached-forever version key (root of the "map fix never held" class). Now calls `scripts/autobump.py` before upload and derives the live-check version dynamically from marketsquare.html (was a stale hardcoded "v395"). Broadened the RG-0013 assertion to accept `autobump.py` as a valid bump mechanism (previously only matched the PowerShell inline expr).
- **RG-0011 promoted OPEN→LOCKED** (country codes ISO + map filenames match, all markets — was READY TO LOCK).
- **Result:** `python3 scripts/regression_ledger.py` → 13 entries · 10 holding · 0 REGRESSED · 3 open (RG-0003/0004/0006 = pre-existing structural-currency defects, untouched) · exit 0. SCAN-28/29 now UN-gated (auto-ship) for the next Fixer.
- Files: `scripts/regression_ledger.py`, `deploy_frontend_nops.bat` (+ `.bak-<ts>` beside each). No app/server change.

## 29 Jul 2026 — COST SWEEP CLEANUP: _to_delete_base40_proof neutralized (Cowork session, David-directed)
- Source: nightly-cost-compliance-sweep (29 Jul) flagged MarketSquare/_to_delete_base40_proof/ (bea_main.py + ms.js, ~1.8MB) as dead weight inflating the paid call-site inventory and Sonnet-registry INFO count. David: "Please fix/remove these dead artifacts/issues."
- Confirmed via git log (added by deploy auto-commit a740061, 28 Jul 18:57) and `git grep` across the repo: never imported or referenced by any app code path — safe to remove.
- **True delete was not possible**: this session's Projects mount is a FUSE/virtiofs bridge that blocks unlink/rename on existing files (rm, mv, git's own index.lock cleanup all fail "Operation not permitted" — same class of issue as the documented Edit/Write truncation bug). Worked around by overwriting both files' content with a short tombstone comment (explains what happened, points at git history commit a740061 for recovery, and tells David how to fully delete the folder from a native Windows terminal/Explorer if he wants it gone from disk entirely).
- Re-ran cost_compliance_sweep.py after the tombstone: paid call-site inventory back to 114 hits (was 121), info count back to 27 (was 30), 0 critical / 0 warnings unchanged throughout.
- Note: a stale `.git/index.lock` was found in the repo during this cleanup (pre-existing, not created by this session) and could not be cleared for the same FUSE unlink reason — `git add`/`git commit` are consequently unreliable from this mount until David deletes `.git\index.lock` directly on the host. The tombstone content change above is therefore an uncommitted working-tree edit pending that.
...
- **C2C-PHOTOS-1:** 12 missing photos generated (Higgsfield / Nano Banana Pro, 3:2, food-first prompt ordering per parallel-session lesson), each eyeballed before download, zero re-rolls; claimed via claim_photos.py --since freshness guard (12=12, in order); build_journey.py → 38/38 embedded, 0 pending. Verify rule: count data:image/svg (0 = complete) — grepping "photo pending" ALWAYS reads 0 because placeholder tiles are base64.
- **DE-PICKER-1 (marketsquare.html):** Germany added to the Adventures country picker. adventures_de_map.html and the ms.js DE advert card already existed — the missing picker div was the entire reason Bavaria never showed.
- **PHONE-BACK-1 (all 9 adventures_*_map.html + journey_template.html):** standalone/enlarged view gets an amber "‹ Back" (history.back → window.close → trustsquare.co fallback; hidden when iframed — the app overlay supplies Close), plus @media ≤640px hides the two map overlay boxes. Cache-busters bumped fleet-wide; c2c → ?v=6 (nginx immutable rule).
- Shipped 11:01 via deploy_marketsquare.bat (rollback tag ship-20260727-1055, commit b5a2420): all steps [OK], BIT self-test PASS, Cloudflare purged, auto-commit pushed. Smoke: trustsquare.co 200/clean; live v6 map serves the new route; photo popups verified live (Karnak); ‹ Back present.
- NOTE (candidate /fix): typing deploy_marketsquare.bat in cmd returned "not recognized" although the file exists and double-click ran fine — likely NoDefaultCurrentDirectoryInExePath hardening; workaround: .\deploy_marketsquare.bat.

## 24 Jul 2026 — Adventures SUPER ADVERTs: +5 photos each + interactive reserve map, genericised (David)
- Source: David — expand both Adventures exemplars with depth and a mapped safari experience ("WOW for viewers").
- **10 new Higgsfield photos (Nano Banana Pro; brand style — 3:2, golden-hour, faces anonymised).** Game drive (270): lions-off-water, leopard, buffalo, snake-vs-meerkat, rhino — all with the SAME cream Land Cruiser via reference-image consistency. Lodge (271): bushveld bar w/ sundown, night braai, boma fire, stone lounge, sunrise deck. Curated to 1200x805 / 150–205 KB in assets/super/ (sup_advexp_4..8, sup_advacc_4..8). Uploaded by deploy step 3d.
- **DB wiring — scripts/expand_adventure_super_photos.py:** idempotent, dry-run-first, DB-backed. Appends the 5 new photos to each listing's photo_urls + [photos:] gallery (270 & 271 go 3->8). Armed as deploy **Step 3f** via RUN_ADV_PHOTOS_ONCE.flag (scp + dry-run + --apply + auto-remove). Proven against the 0830 live-DB copy; re-run is a clean no-op.
- **ADV-MAP-1 — ms.js + adventures_reserve_map.html:** self-contained Leaflet map (inlined lib, base64 photos), pins for gate/lodge/waterhole/sundowner + Big Five with photo popups, route toggle + legend. Embedded as an "Explore the reserve" iframe panel in the detail view, scoped to (l.super_example && isAdv). Map ships to /static via deploy **Step 3c-map**. node --check clean; iframe embed verified headless at mobile width (10 pins render).
- **ADV-SAFE-1 — Option A safe pass (David, standing rule SO-1):** exemplars must never be mistakable for a real, named place. Genericised BOTH listings off Dinokeng Game Reserve — "Dinokeng" removed from title (270 "— Gauteng"), description ("the reserve's waterhole"), the suburb field (cleared) and the whole map (now "a Big Five reserve in Gauteng, ~40 min north of Pretoria"; renamed adventures_reserve_map.html; map kept DELIBERATELY illustrative, not surveyed). Gauteng/Pretoria kept as generic locators. Folded into the same idempotent one-shot; proven + idempotent on the 0830 DB copy. New STANDING_ORDERS.md SO-1 makes this the rule for all future super adverts; anything based on a real place is flagged to David to offer the owner the lead advert.
- **ADV-MAP-GESTURE-1 — mobile gesture fix (David + 2 testers):** the embedded map hijacked one-finger swipes (users wanted to scroll the advert). Inlined Leaflet gesture-handling: on touch the map needs TWO fingers to move/zoom and one finger scrolls the page; on desktop normal drag + ctrl-scroll to zoom; a "use two fingers to move the map" hint shows on a blocked gesture. Proven by touch simulation — one-finger drag pans the map 0px (dragging disabled), two-finger zoom stays live. Map file only (adventures_reserve_map.html); ships as before.
- TO SHIP: `/TSL` (or `/ship`) uploads ms.js + map + photos and auto-applies the flagged DB script; then `/pulse`.

## 24 Jul 2026 — Step 1 (pin the substrate) reviewed: open-weight flip validated READY
- Durability roadmap step 1. Finding: the seam is already built. Every LLM call routes through `ai_provider.complete(provider=_ts_active_provider())`; the active provider is a LIVE DB flag (`launch_switches.ai_active` → dashboard "AI Providers" card → `/admin/flags`, instant, no restart, reversible, cache busted on write); and Scaleway EU / `mistral-medium-3.5-128b` is already golden-set-passed (7/7 text, 2/2 vision, 18 Jul). Flipping `ai_active` → `scaleway` makes the open-weight model primary with Anthropic + OpenAI as the any-of failover. Rung 1 → 2 is a switch David already owns. Audit: the deployed backend (bea_main.py) is seam-clean except one guarded site (below); `main.py` is a non-deployed legacy twin.
- Two residuals for a TOTAL cord-cut (neither blind-edited — both need care, not a box-tick):
  1. **vision-draft** (~bea_main.py 13037) is the one un-migrated raw call site, deliberately guarded to the Anthropic wire format. Migrating it to the seam (`task="vision"`) is an async→sync, untestable-from-cloud change to a live feature — do it alongside a vision golden-set run, not blind.
  2. Dashboard AI-Providers confirm copy still reads "golden-set eval pending — outage use only"; stale since the 18 Jul Mistral eval passed. Refresh when David declares Scaleway production-primary (a product/risk stance, his to make).
