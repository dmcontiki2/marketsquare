...
|---|------|-------|--------------------|--------|--------|
| L8 | **The `design` task tier is DECLARED but has no live caller.** RUL-013 sent design work to the allocated design agent from 1 Sep; `AI_BASELINE.json` carries `tiers/design` (openai gpt-5.6-sol, scaleway standby, ceiling 4000) and says in its own `_doc`: *"Envelope DECLARED, not measured — no live caller exists yet."* So the lane David was told would take design work has, 18 days after it opened, never been called. | [C] | Wire the first real caller and re-derive the envelope from real call shapes, then the golden run its gate requires. Technical throughout — no part of this is David's. Recorded here (not as a new ledger entry) because a concurrent lane is writing `scripts/` and RG numbering is not safe to race. **RE-PROBED 2026-09-23 17:0xZ — STILL OPEN, now 22 days declared with no caller.** `bea_main.py` line 22646 carries `_MAINT_TIER_LADDER = ("design", "reason", "fast", "vision", "triage")` and `/admin/maint/brain` accepts `task="design"`, so the rung exists — but `grep task="design"` across every `.py` in the tree returns **zero callers**, and `/dashboard/maint` shows the last twelve agent runs every one on **tier `fast`**. The envelope is still `DECLARED, not measured` in its own `_doc`. The concurrent-lane reason to hold off is unchanged and was true again today: `scripts/regression_ledger.py` and `scripts/maintenance_agent.py` were both being written during this run. | 2026-09-19 | RUL-013 · AI_BASELINE.json `tiers/design` · closure of D14 |
| L9 | **RUL-162 and the whole 23 Sep Quick-audit list exist in a ruling row and one working file — not here, and not committed.** David's airport session on 23 Sep ruled the two-layer language design (app speaks the user's language, advert speaks the lister's, one extra language with a back-translation she approves), replaced Sesotho with Sepedi on South Africa's list, and approved per-country lists for all nine active countries. The same session logged **twelve Quick-listing defects (Q1–Q12)**, four of which break the flow on the live product: Q1 Quick is a one-way door (hand-over must land her signed in, on her draft), Q2 the app re-asks what Quick already knows, Q3 two different scores for one advert (Quick 60 vs app 80), Q4 published-but-invisible until refresh. **PROBE 2026-09-23 17:0xZ:** `grep RUL-162 OPEN_LOOPS.md` → **0**; `genie/LANG_QUICK_BRIEF.md` and `genie/LANGUAGES_QUICK_AUDIT_BOARD.html` are **untracked**, `RULINGS.md` is **uncommitted**. | [C] | Q1–Q4 first — they are live defects on the seller's path, not polish — then Q5–Q12, then the language layer tester-only per RUL-162(d). **Not started this run and deliberately so:** another session was writing `genie/`, `scripts/maintenance_agent.py`, `scripts/regression_ledger.py` and `RULINGS.md` while this stand-up ran (mtimes 16:43–17:06Z), and RUL-140 says a lane stands off a scope another lane is in. Recorded, not raced. | 2026-09-23 | RUL-162 · genie/LANG_QUICK_BRIEF.md · RUL-149 (amended) · RUL-160 |
| L10 | **Two instruments convict the app of faults they were never able to look at — one fixed this run, one diagnosed and not touched.** `scripts/rulings_check.py` reported **38 FAIL / "the blind spot is live"** on a clean tree: 38 of its assertions point at `../CityLauncher/…` and `os.path.join(PROJECTS, "CLAUDE.md")`, which exist on David's machine (`Projects/` listed 23 Sep: CityLauncher, Auction and 23 more) but are not mounted on this task. **FIXED (VANTAGE-BLIND-1):** an unreachable path is now `NOT CHECKED`, never FAIL; an in-repo file that is genuinely gone still FAILs. Same tree, same minute: old code exit 1 / 38 FAIL, new code exit 0 / **0 FAIL, 24 WARN, 38 NOT CHECKED** — matching the 0 FAIL that `c7a857f` got from a full mount on 20 Sep. Test `scripts/test_vantage_blind1.py` pins both halves. **`scripts/regression_ledger.py` has the identical defect and was NOT touched:** it printed **6 REGRESSED** today and five of those (RG-0229, RG-0230, RG-0252, RG-0399, and RG-0245's vantage limb) are the same unmounted siblings — while RG-0368/0370/0403/0418/0419 already demote to UNVERIFIED for exactly that reason, so the doctrine is in the file and five entries miss it. A false REGRESSED prints "Do not deploy over this". | [C] | Port VANTAGE-BLIND-1 into `regression_ledger.py`'s five convicting entries, and wire `scripts/standup_freshness.py` (STANDUP-WATCHDOG-1, written and proven this run) into the maintenance loop beside the backup lane — one line, `report["standup"] = …`. **Both deliberately left unwritten:** `scripts/regression_ledger.py` and `scripts/maintenance_agent.py` were being edited by another lane while this stand-up ran (mtimes 17:06Z), and RUL-140 is a stand-off, not a preference. Next session holding those files with a quiet tree lands both. | 2026-09-23 | VANTAGE-BLIND-1 · RG-0401 · RG-0420 · L13's `vantage:` rule · scripts/standup_freshness.py |
