...
| L11 | **Three instrument fixes shipped this run have no ledger entry and no changelog fragment — they exist in code and in this file only.** LEDGER-VANTAGE-BLIND-1 (`scripts/regression_ledger.py` + `scripts/test_ledger_vantage_blind1.py`), BIT-EDGE-BLIND-1 (`bit/bit_runner.py` + `scripts/test_bit_edge_blind1.py`) and BIT-NS-1 (`scripts/golden_seam_v2.py`) are all in the tree, compiling, each with a test that fails on the old code. None has an RG number and none has a `changelog.d/` or `status.d/` fragment, because `scripts/regression_ledger.py`, `changelog.d/*` and `status.d/*` are inside lane `lang-quick-2026-09-23`'s work lock, taken 19:55:21Z. This is rule 2's failure mode reached honestly — by standing off a lock per RUL-140, not by forgetting — which is exactly why it is written here instead of being carried in a session transcript. | [C] | Next session holding a quiet tree: add the three RG entries (the doctrine text is already in the code comments, ready to lift), write the changelog and status fragments, and let the one writer fold them. Nothing is waiting on David and nothing is waiting on a decision. | 2026-09-23 | LEDGER-VANTAGE-BLIND-1 · BIT-EDGE-BLIND-1 · BIT-NS-1 · RUL-140 · rule 2 of this file |
| L12 | **RG-0426 tells David to go buy an AI key he already owns, and it is the only thing left in the ledger still pointing at him.** The entry is OPEN and FAILs with *“only 1 AI vendor key is live on the box (ANTHROPIC_API_KEY) — the ranked failover has nowhere to fall to”*, and its scope text ends *“Provisioning the key is David's — it is a vendor credential, and the money behind it is his call.”* **PROBED ON THE BOX 2026-09-23 20:2xZ and the premise is FALSE.** `/var/www/marketsquare/.env` carries **OPENAI_API_KEY, 164 characters** (name and length only — no value read into any log). The systemd unit sets `ANTHROPIC_API_KEY` via `Environment=` and never exports the .env, which is exactly why `ai_provider.envkey()` exists: its own docstring says *“fall back to the server .env file (the systemd unit does not export it to this process — ENVKEY-1, 17 Jul 2026)”*, and `ai_provider.py:161` reads the OpenAI key through it. So inside the running service **both** lanes resolve — anthropic from the unit, openai from the file. Independent corroboration, already published and needing no credential: `/dashboard/maint` reports `brain_lane: openai`, `brain_keyed: true`, `brain_probe {ok: true, status: 200}` — an OpenAI call from this box succeeded today. **WHY THE ENTRY IS WRONG, AND IT IS THE DAY'S PATTERN AGAIN:** it reads `/proc/<pid>/environ`, citing RG-0147's “check at the point of use” — but the point of use is `envkey()`, which by ENVKEY-1's *design* looks in the file the unit does not export. A .env-sourced key can never appear in that probe, so the check can only ever return False and can never pass. Fourth consecutive stand-up to find a row aimed at David that was already a fact, and the fourth instrument today caught convicting from the wrong vantage. | [C] | Re-aim RG-0426's probe at `ai_provider.envkey()` (the point of use) instead of `/proc/<pid>/environ`, keeping the assertion's teeth: no reachable standby lane must still FAIL. **Not done this run:** `scripts/regression_ledger.py` is inside lane `lang-quick-2026-09-23`'s work lock (taken 19:55:21Z) and RUL-140 is a stand-off. Nothing here is David's and nothing needs buying — that is the whole finding. *If* the re-aimed probe then shows the OpenAI account unfunded, the money is his and this becomes a [D]; it is not one today. | 2026-09-23 | RG-0426 · RG-0147 · ENVKEY-1 · ai_provider.py:161 · /dashboard/maint brain_probe |
| L13 | **The BIT board exists as TWO separate files six weeks apart, and nothing asserts they agree.** `bit/bit_runner.py` is what the stand-up runs (through the Cloudflare edge); `ops/bit/bit_runner.py` is what the deploy manifest ships to `/var/www/marketsquare/bit/` and what `trustsquare-bit.timer` runs **every 15 minutes** on the box. Different inodes, no link. **PROBED 2026-09-23 20:3xZ:** the served copy is dated **11 Aug 2026** — six weeks stale — and lacks even `--verbose`/`--json`. The divergence is NOT accidental in one direction: the ops copy is a deliberate server variant (it probes `localhost:8000` and carries `BIT_AA_BASE`), which is why BIT-EDGE-BLIND-1 was fixed only in `bit/` this run and **must not** be copied over it — the server copy is immune to the edge refusal by construction, and blind-copying would have destroyed its localhost wiring. The loop is that the shared check logic has no single writer and no assertion that the two agree, so a real fix to either board silently fails to reach the other. | [C] | Factor the shared checks into one imported module with the two thin vantage wrappers on top (edge vs localhost), then a ledger entry asserting both boards resolve the same check set — the ONE-WRITER pattern `scripts/changelog_compile.py` and `status_compile.py` already use. The manifest line mapping `ops/bit/bit_runner.py` onto the served `bit/bit_runner.py` is the thing that has to change last, and `ops/autodeploy/deploy_manifest.txt` is inside lane `lang-quick-2026-09-23`'s lock this run. | 2026-09-23 | BIT-EDGE-BLIND-1 · ops/autodeploy/deploy_manifest.txt:170 · trustsquare-bit.timer · RG-0421 (ONE WRITER) |
| L14 | **E2E-HMI-1 (24 Sep): two legs of the human-interface walk were not walked** — (a) buyer intro → seller accept → 1T burn → contact reveal, and (b) a Local Market seller's no-show complaint button (not built: no accepted LM intro exists to prove it on). David declined a QA Tuppence grant on 24 Sep, so both wait on that. Everything else on the board `E2E_HMI_WALK_2026-09-24.html` is fixed and read back live (commits b951503, 3450af9, 57a9f0b). | [D] | Approve 1-2 QA Tuppence (granted and reversed), then walk both legs and build/prove the LM no-show button in the same session. | 2026-09-24 | changelog.d/2026-09-24-e2e-hmi.md |
