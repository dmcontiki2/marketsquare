...
|---|------|-------|--------------------|--------|--------|
| L5 | **Fix-agent: harness honesty FIXED + `propose_patch` now feeds the brain the real file** — the two honest Tier-2 misses were a blind prompt (no file shown); a deterministic finder + file-context added, verified locally. Agent still OFF. | [D]+[C] | Deploy this release -> re-run the guarded Tier 2. If it PASSES it is genuinely earned -> decide arming; if the diff still won't apply, [C] adds the full-file-rewrite fallback. | 2026-08-09 | this session |
...
| D8 | **Stays/B&B — BUILT, awaiting the push.** 15 photos, 3 born-clean adverts (`migrations/009`), 3 phone cards, the Stays email track's 3 FAKE cards + 3 Unsplash hotlinks replaced with real deep-linkable ones, and the 4-layer ZA map `adventures_za_map.html` (route/heritage/stays/partners, inlined Leaflet, intros 1T, partners inert-pending) — all on disk, gates green, in the manifest. | [D] | David: run the deploy shortcut (code lane). Photos + phone cards need `media_push.bat`, which is BLOCKED by DW-020 (SSH down) — adverts will show broken heroes until that clears. Then set the 3 ids in the map's `STAY_IDS` + run flip_showcase_hrefs.py. | 2026-08-02 | changelog.d STAYS-SHOWCASE-1 + -complete |
