...
| D12 | **One deploy clears the board** — carries maint-close batch (RG-0049, escalation brief, guidelines, arming assets), migrations/011 (Tier-2 rehearsal verdict) + 013 (server BIT timer → BIT UNKNOWN ×2 go green) + VIZ-MAPS-4. | [D] | Say "ship" / run the deploy shortcut — or tonight's 05:45 nightly carries it if gates are green. THEN: read static/maint/b4_tier2.json; if READY, arm via MAINT_ARMING_RUNBOOK.md (one paste). | 2026-08-11 | changelog.d 2026-08-11-maint-close |
