...
**Last reconciled: 2026-09-18 02:2xZ (stand-up, mechanical — SO-3/RUL-037).** Previous
...
|---|------|-------|--------------------|--------|--------|
| L3 | **SCOREBOARD-1 shipped-not-live** — agent + guards + nightly wiring in repo (7/7 tests); probes OFF until enabled. **RE-PROBED 2026-09-18:** no scoreboard surface answers at the origin (`/dashboard/scoreboard`, `/scoreboard`, `/static/scoreboard*` all 404), so it is still not live. The code has ridden many deploys since 3 Aug — the deploy was never what was missing. | [C] | **Owner corrected [D]→[C] 2026-09-18 (RUL-095).** Running a bat is no longer a David click. The real blocker is that `enable_scoreboard.bat` is (a) INTERACTIVE — `choice /M` + `pause`, which would hang the host-queue worker — and (b) not on `host_queue/ALLOWLIST.txt`. Work is: give it a `--yes` non-interactive path, add the allowlist line, then queue it. Not executed this run: it ALTERs and UPDATEs the live `launch_switches` table and starts nightly spend, so it ships through the normal gate, not off a 02:00 stand-up. | 2026-08-03 | CHANGELOG SCOREBOARD-1 |
...
| L9 | **Maintenance heartbeat missed a tick — NEW 2026-09-18, found by this stand-up.** `/dashboard/maint` last posted **2026-09-17T17:21:01Z**; at 02:2xZ on 18 Sep that is **~9 hours**, against a prior cadence of ~6 h (17 Sep runs at 06:08, 11:20, 17:21 UTC). A run due ~23:20Z did not post. Under the stand-up's own rule (~12 h with no post = amber) this is **not yet amber**, and it is recorded now rather than at 12 h because a missed tick is the cheapest moment to catch a dead scheduler. Consequence already visible: the dashboard's `code` field still reads `62de7ed` (the 17 Sep release) although `origin/deploy` advanced to `33387d7` at 17:51Z — the field is a stale snapshot from the last run, not a deploy fault. The deploy itself is confirmed landed (see the run notes). | [C] | **ACTED 2026-09-18 02:22Z:** queued `run_bat MarketSquare\maint_host.bat` on the host queue (`20260918-022221-520_run_bat_marketsquare-maint-host.req`) — the SHADOW lane, which by design never sets `MAINTENANCE_AGENT_ENABLED` and cannot commit, send or delete. Read `host_queue/done/` for the result before calling this closed. If the heartbeat returns, the scheduler missed one tick; if it does not, the Windows scheduled task is dead and that is the real fault. Not diagnosable from here: `device_bash` is a Linux VM, so the host's `schtasks` state cannot be read from the sandbox. | 2026-09-18 | this stand-up |
| L10 | **The published breach clause is wrong for the UK, and it is live now.** `/privacy` §7 commits to notifying "within 30 days" — the POPIA-shaped answer. **UK GDPR Art 33 requires the ICO inside 72 hours.** For a UK user the site publishes a commitment to miss the statutory deadline by 27 days. Found 18 Sep by reading privacy.html against EULA Schedule A. AU differs again (30-day *assessment*, notice "as soon as practicable"); US states are mostly "without unreasonable delay". | [D] | Replacement §7 is drafted in `PRIVACY_SUPPLEMENTS_GB_US_AU_DRAFT_2026-09-18.md` (Finding 2) and is self-contained — it can land ahead of the supplements. NOT edited in place: published legal text, external review track. | 2026-09-18 | this stand-up |
| L11 | **Two published documents route privacy rights requests to different addresses.** `/privacy` §9 says write to **support@**; EULA v1.16's contact table says **legal@** for "Privacy / POPIA / data requests". A data-subject request sent to whichever is not monitored is a request denied, and the clock (30 days POPIA/AU, 1 month UK, 45 days CA) runs regardless. **Neither address was probed for deliverability this run** — saying so rather than assuming. | [C] | Confirm which mailbox is monitored, prove it receives, then make both documents say the same thing. The drafted supplements use `legal@` to match the EULA; if `support@` is the live one, that is a one-word change in three places. | 2026-09-18 | this stand-up |
| L12 | **France and Portugal have the same defect D4 just fixed for GB/US/AU.** EULA v1.16 carries Country Schedules D (France) and E (Portugal); Schedule D5 makes its own "as described in the Privacy Policy" cross-reference to GDPR and the Loi Informatique et Libertés. The privacy policy contains no EU content. **GDPR Art 27 (EU representative) is therefore live too, on the same reasoning as the UK one.** Recorded rather than drafted: David scoped 18 Sep to three countries, and widening a scope he just set is not mine. | [D] | Say whether to draft the EU supplement (France + Portugal) on the same pattern. The Art 27 EU representative is a separate spend decision. | 2026-09-18 | this stand-up |
| L13 | **`/dashboard/maint` reports the LAST RUN's vantage as the agent's standing state.** Proven 18 Sep by accident, then deliberately: at 02:20Z the card read `armed:true armed_switch:true mode:LIVE` (the server's own scheduled run); after the host-side SHADOW run this stand-up queued at 02:22Z, the same card read `armed:false armed_switch:false mode:SHADOW`. **Nothing was disarmed.** `armed_switch` is `KILL`, read from the env of whatever process posts — and `maint_host.bat` never sets `MAINTENANCE_AGENT_ENABLED`, by design. So a host run overwrites the server's arming state with a value it cannot observe. This is exactly RG-0382's contract (a vantage LIMIT must stay distinguishable from a FAULT) applied to the brain field on 17 Sep and never extended to the arming fields beside it. Harm: a reader — David, a stand-up, or the next session — concludes the maintenance agent is disarmed when it is armed and running, or re-arms what was never off. | [C] | Producer fix in `scripts/maintenance_agent.py::_post_heartbeat`: when the run's own vantage probe says off-box (`BRAIN error_kind == 'vantage:local-only'`), post `armed`/`live`/`armed_switch` as NOT_EVALUATED rather than false. Consumer half is in `dashboard.server.html`, which a concurrent lane held all through this run (DEVICE-NOLAPSE-1) — **not edited, per SO-5.** Mine to ship once that lane is clear; ledger entry to follow the fix, not precede it. | 2026-09-18 | this stand-up |

## ⚪ DECISIONS AWAITING DAVID / COUNSEL — ranked

| # | Decision | Owner | Single next action | Opened | Source |
|---|----------|-------|--------------------|--------|--------|
| D4 | **Privacy supplements — SCOPE ANSWERED by David 18 Sep 2026: "all three" (GB · US · AU). DRAFTED SAME SESSION, not applied.** Text at `PRIVACY_SUPPLEMENTS_GB_US_AU_DRAFT_2026-09-18.md`: Supplement A (UK GDPR/DPA 2018 — lawful-basis table, Art 22 position, IDTA transfers, ICO), Supplement B (CCPA/CPRA notice-at-collection table, SPI, no-sale, 45-day response, + 8 other states with an appeal route), Supplement C (Privacy Act 1988 / APPs 1,2,3,5,6,7,8,11,12,13 + NDB + OAIC). Not edited into `privacy.html`: that document has an external review track, same routing as the Buzz clause on 14 Sep. | [D] | **Review and land it.** Three things must be resolved in the same revision, all named in the draft: the UK Art 27 representative placeholder (spend + vendor = yours, RUL-009); the §7 breach clause (see L10); and the `legal@` vs `support@` split (see L11). | 2026-07-23 | STATUS.md S149 · David 18 Sep |
