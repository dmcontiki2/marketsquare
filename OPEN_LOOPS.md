...
| D8 | **Stays/B&B — now the 4-LAYER MAP vision (David, 5 Aug):** B&Bs are one of four map layers — ROUTE pins to drive between + HERITAGE sites + near-by B&B INTRO options (the Tuppence seam) + Travelpayouts PARTNER referrals (pure link-out) — together one interactive phone map with geolocation and browse buttons. Working concept built: `ADVENTURES_FULLMAP_CONCEPT.html` (in Visuals gallery). Email-showcase trio still needed and still photo-gated. | [D]+[C] | David reviews the concept + generates the 3 B&B photo sets (Higgsfield session); Claude wires adverts + map layers end-to-end. | 2026-08-02 | this session |
