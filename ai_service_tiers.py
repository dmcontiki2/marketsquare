...
# ── Paid-feed AI class — S3 tier-gate (Free-Tier AI Cost Risk Report, 16 Jun 2026) ──
# These are the expensive AI functions that draw on a CONTRACTED/CONSUMPTION data
# feed (B7-class). Decision S3: reserve this class for the $20 Pro seller tier, where
# the 5T = $10 charge covers Sonnet-plus-feed cost. Free / Starter / Agency are blocked.
# Free-data / Haiku / OSM-Nominatim functions are NEVER in this set — cheap AI stays
# open to free users (the conversion hook). The gate is enforced at /tuppence/ai-commit.
#
# NOTE: today every paid feed is flag-OFF (DEFAULT_PROVIDERS), so these functions run on
# free/own data and the gate is dormant-but-present. Listing them here means the gate
# activates automatically the moment a feed is contracted — no second code change needed.
...
def requires_paid_feed(function_id: str) -> bool:
    """True when this AI function belongs to the Pro-gated paid-feed class (S3)."""
    return (function_id or "").strip() in PAID_FEED_FUNCTIONS


def tier_may_run(function_id: str, seller_tier: str) -> bool:
    """Gate check: may a user on `seller_tier` fire `function_id`?
    Non-paid-feed functions are open to everyone; the paid-feed class is Pro-only."""
    if not requires_paid_feed(function_id):
        return True
    return (seller_tier or "free").strip().lower() in PAID_FEED_ALLOWED_TIERS
