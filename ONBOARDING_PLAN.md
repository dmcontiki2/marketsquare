...
**Corrected 13 Sep 2026 (measured): the lane did not run dry — it was switched off, and the Texas lead is dead.** The 13 Sep 00:10 wave sent nothing ("no armed city has anyone to send to"). Not supply: at 06:17 on 12 Sep a session executed the jurisdiction gate by disarming every US, UK and Australian wave entry — 72 of them, the whole US state lane included — because the 20 Aug law notes carried the US/UK/AU research in an appendix rather than under a numbered heading, and the gate tests headings. That contradicted three standing rulings made with those notes in hand (RUL-071, whose own city map marks every US/UK/AU city as outreach-covered; RUL-074, "all three countries are outreach-covered per the 20 Aug law notes"; RUL-082 and RUL-059, which armed and launched them). Nothing legal changed on 12 Sep. Fixed 13 Sep as a class: the research now sits under ruled sections 10–12 of the law notes, the gate reads the send engine's own state map, the 72 entries are re-armed, and a ledger entry (RG-0361) goes red if the gate and the rulings ever disagree again. Measured after the re-arm: **854 sendable people** — Maine 536, Alaska 137, Montana 118, Colorado 63 — every other armed city at zero. **The Texas real-estate licence file is NOT a lead:** probed on the Texas open-data portal, 20 columns, no email — Texas stripped addresses, phones and mailboxes from the public file by statute. Official licence files are now assumed to carry no mailbox unless one is seen; the register kinds still open are member directories that publish a mailbox (associations, chambers, permit-holder lists), and the sandbox CAN read Socrata open-data portals (data.<state>.gov), which is where any state file with a mailbox would show up cheaply.
