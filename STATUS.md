...
- **S5 DONE (verified 2 Jun · Gate-2, deployed).** Payment grants now fail-closed: `_payment_grants_allowed()` guards all 4 grant paths (live key always; test/unset key only if `ALLOW_TEST_PAYMENTS=1`) + `/payment/verify` made idempotent (was double-creditable). Live: sk_test + flag unset → grants refused; smoke 39/39. **Server runs old-base + S5; local repo = committed TVS (steps 1-2) + S5** — TVS was committed but never deployed (still pending). Ops to finish money path: set `PAYSTACK_WEBHOOK_SECRET` (reliable webhook credit path, currently unset → webhook 400s) + `ALLOW_TEST_PAYMENTS=1` to run M1/M2 test-card flow. ⚠️ stale `.git/index.lock` may block commit (clear it).
- **M0 part 1 DONE (verified 2 Jun · BEA, non-breaking).** `GET /listings` now supports `?page=&page_size=` (default 200 = unchanged) with `LIMIT ? OFFSET ?` on both branches — no more 200-truncation. Deployed (server old-base+S5+M0; backup `main.py.bak-20260602-m0`); default feed returns full 65 unchanged, page1/page2 distinct; smoke 39/39. Local repo = TVS+S5+M0 (sha-verified, 10928 lines). **Next: M0 part 2 = FEA infinite scroll in `ms.js`** (request small pages + append at grid bottom; stop when a page < page_size).
