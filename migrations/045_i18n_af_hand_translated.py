#!/usr/bin/env python3
"""045_i18n_af_hand_translated.py -- I18N-AF-1 (20 Sep 2026).
...
def main():
    if not os.path.isfile(DB):
        print("045: no database at %s -- nothing to do" % DB); return 0
    if not os.path.isfile(SRC):
        print("045: %s not on the server (not in the deploy manifest?) -- skipped" % SRC); return 0
    ...
    print("045: %d checked Afrikaans phrases, %d kept in English" % (len(rows), len(kept)))
    if not APPLY:
        print("045: dry run -- pass --apply"); return 0
    ...
    try:
        ...
        print("045: applied; i18n_cache now holds %d Afrikaans phrases" % n)
    finally:
        conn.close()
    return 0
