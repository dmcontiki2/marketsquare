...
async function confirmTopUp(){
  ...
  try {
    ...
    // AI-pack checkout param retired (CC-002; packs retired, Canon Addendum 1)
  ...
  document.getElementById('screen-seller-cv').innerHTML = `
    ...
    <div class="cv-sticky">
      <button class="cv-cta" onclick="openModal('${l.id}')">
        ...
        Request Introduction · 1T held · used on accept
      </button>
    </div>`;
  goTo('seller-cv');
}
  ...
  document.getElementById('screen-detail').innerHTML=`
    ...
    <div class="sticky-cta">
      <button class="cta-btn ${isCommit?'commit-cta':'queue-cta'}" onclick="openModal('${id}')">
        ...
        <span class="cta-cost">· 1T held · used on accept</span>
      </button>
    </div>`;
  goTo('detail');
  loadDetailWonders(l);
...
function openModal(id){
  ...
  document.getElementById('modal-cta-btn').textContent=isCommit?'Submit Introduction Request':'Join Queue · 1T held · used on accept';
  ...
  if (tnNotice) tnNotice.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> 1 Tuppence ($2) committed when you request — used only when the seller accepts; released if declined';
...
let sobState = {
  ...
  selectedTier: 'free' // 'free' | 'standard' | 'professional' | 'business' | 'elite'
};
      ...
      return `
        <div style="border-radius:16px;overflow:hidden;background:linear-gradient(160deg,#1e2d4a 0%,#22304d 60%,#1e2a3a 100%);box-shadow:0 8px 32px rgba(0,0,0,.3);margin-bottom:8px;">
          ...
          <div style="padding:12px 16px 16px;">
            <div style="width:100%;background:rgba(139,92,246,.25);border:1.5px dashed rgba(139,92,246,.4);border-radius:50px;padding:14px;text-align:center;font-size:14px;font-weight:700;color:rgba(139,92,246,.6);">👥 Join Queue · 1T held · used on accept</div>
  ...
  for (const draft of sobState.drafts) {
    try {
      ...
      await fetch(BEA_URL + '/users', {
        ...
        body: JSON.stringify({ email, name: sobState.name })
      }).catch(() => {});
...
// ── WALLET: AI feature credits (legacy coach block — title canon CC-002) ──────────────────────────────────
...
async function lmOpenDetail(listingId) {
  ...
  try {
    ...
    el.innerHTML =
      ...
      `</div>` +
      `<div class="sticky-cta"><button class="cta-btn queue-cta" onclick="openLMModal()">👥 Request Introduction <span class="cta-cost">· 1T held · used on accept</span></button></div>`;
  } catch(e) {
...
const HIW_FEATURES = [
  ...
  { label: '1T committed on request · burned on accept', ans: ['1T = $2','1T = $2','1T = $2'], tone: ['yes','yes','yes'],
