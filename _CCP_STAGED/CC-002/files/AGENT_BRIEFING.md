...
- **Advanced AI features** are Tuppence-priced **per use** through the HOLD ledger (`POST /tuppence/ai-commit` + `POST /tuppence/ai-settle`); **in-app AI guidance is free**. AI-uses packs are RETIRED (Canon Addendum 1 rev 2 · 6 Jun 2026); wallet shows AI feature spend as "AI feature credits — not used for introductions".
...
| POST | `/payment/initialize?tuppence=N` | None | Paystack checkout — Tuppence top-up (AI-pack param retired, CC-002) |
| GET | `/payment/verify` | None | Verify payment · credit Tuppence |
| GET | `/advert-agent/status?email=` | None | Coach status (legacy session counters retired — CC-002) |
...
| POST | `/advert-agent/buy-pack` | None | **410 Gone** — packs retired; advanced AI is per-use via the HOLD ledger |
...
| POST | `/dev/credit?email=&tuppence=N` | API key | **Dev-phase only** — seed account with Tuppence, no Paystack · **REMOVE BEFORE LAUNCH** |
