...
- The **requester** pays (the service consumer — the buyer, for introductions). Tuppence is **committed on request → burned on delivery (seller accepts) → released on decline/expiry** (Tuppence HOLD, Codex v4.8 / CC-001).
...
|---|---|---|---|
| `marketsquare_v8_6b.html` | v8.6b | trustsquare.co | Frontend agent |
| `marketsquare_admin_v1_1.html` | v1.1 | trustsquare.co/admin.html | Admin agent |
| `Solar_Council_Codex_v4_0.docx` | v4.0 | Session upload only | Architect agent |
...
**Owns:** Codex (`Solar_Council_Codex_v4_0.docx`), system design, rule arbitration.
...
- `renderFeatured()` — renders featured carousel on home
...
- `LISTINGS[]` — all listing objects
- `SELLERS[]` — anonymous seller objects (no name/email exposed to JS below fold)
...
**Magic claim link:** After publishing, a unique URL is generated for the seller. The seller taps the link, enters their email, and receives a secure link to claim their listings. (n8n email automation is pending — currently sent manually.)
...
**Photo upload:** Photos POST to BEA `/listings/photo`, returns `thumb_url` + `medium_url`. nginx limit is 20MB per upload. File size is shown on selection; upload is blocked if over limit. Error handling is in v1.1.
...
| PUT | `/intros/{id}/accept` | API key | Accept intro · burns the buyer's committed 1T hold |
...
| nginx | Reverse proxy · 20MB upload limit | ✅ Running |
...
## 8 · What Is Pending (Session 2 → Session 3)

| Priority | Item | Owner |
|---|---|---|
| 1 | Maroushke real listings — add actual property listings via admin tool | Admin agent |
| 2 | n8n email notifications — buyer emailed on accept/decline | Architect / BEA |
| 3 | Hetzner Object Storage — migrate photos from local /media | Architect / BEA |
| 4 | GitHub setup for project backup | David |
| 5 | Test start_marketsquare.bat end-to-end | David |
| 6 | Correct Codex — Solis is a ChatGPT/OpenAI persona, not Grok | Architect agent |
| 7 | Correct Codex — 1 Tuppence = USD $2 (currently shows blank in table) | Architect agent |
| 8 | Update all agent CLAUDE.md files to reflect marketplace, not game | Architect agent |
| 9 | Paystack live mode (pending CIPC registration) | Pending |
