...
// R2 pattern: pub-xxx.r2.dev/path/file.jpg → /media/path/file.jpg
    ...
    if (live && live.length > 0) {
      ...
      const mapped = live.map(l => {
        ...
        return {
          ...
          rental_status: l.rental_status || null,
          available_from: l.available_from || null,
          availability_label: l.availability_label || null,
          // CARS-SPEC-1: structured vehicle columns (server scrub means only
          // confirmed+attested values ever arrive on public reads)
          make:          l.make          || null,
          model:         l.model         || null,
          variant:       l.variant       || null,
          vehicle_year:  l.vehicle_year  || null,
          year:          l.vehicle_year  || null,   // existing cars filter reads l.year
          mileage_km:    (l.mileage_km || l.mileage_km === 0) ? l.mileage_km : null,
          transmission:  l.transmission  || null,
          fuel_type:     l.fuel_type     || null,
          body_type:     l.body_type     || null,
...
async function _msInit(){
  ...
  if(sp.get('signin')){
    ...
    (async function(){
      try{
        const r = await fetch(BEA_URL+'/auth/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:_tok})});
        ...
        if(r.ok && d.email){
          ...
          showToast((d && d.detail) || 'That sign-in link did not work — request a new one.');
          goTo('signin');
        }
...
// ── Gallery thumbnail strip sync (category-neutral — CARS-SPEC-1 Part A) ────
// Canonical gal* names; Cars, Adventures (and future categories) share one
// component. DOM ids/classes (adv-thumb*) unchanged so existing CSS holds.
function galThumbClick(id, idx) {
  const strip = document.getElementById('pstrip-' + id);
  if (!strip) return;
  strip.scrollTo({ left: idx * strip.offsetWidth, behavior: 'smooth' });
  setTimeout(() => syncGalThumbs(id, idx), 80);
}

function syncGalThumbs(id, idx) {
...
// Back-compat aliases — inline onclick/onscroll strings in rendered DOM keep working
function advThumbClick(id, idx) { return galThumbClick(id, idx); }
function syncAdvThumbs(id, idx) { return syncGalThumbs(id, idx); }

// ── CARS-SPEC-1 · Vehicle quick-spec strip + spec panel (detail view) ───────
// Public payloads are scrubbed server-side (visibility invariant D1): any value
// arriving here is seller-confirmed + attested (or demo). Spec-less listings
// render the legacy chip row byte-identically — old cars pages do not change.
function vehSpecs(l){ try{ const s=JSON.parse(l.vehicle_specs||'{}'); return (s&&typeof s==='object'&&!Array.isArray(s))?s:{}; }catch(e){ return {}; } }
function vehHasSpecData(l){
  if(l.make||l.model||l.variant||l.vehicle_year||l.mileage_km||l.drivetrain||l.fuel_type||l.body_type||l.colour) return true;
  const s=vehSpecs(l); return Object.keys(s).some(k=>k!=='_prov');
}
function vehQuickSpec(l){
  if(!vehHasSpecData(l)){
    // Legacy chip block — exact pre-CARS-SPEC-1 rendering for spec-less listings
  ...
  const tiles=[];
  if(l.mileage_km||l.mileage_km===0) tiles.push(['Mileage', Number(l.mileage_km).toLocaleString('en-ZA')+' km']);
  if(l.drivetrain)                   tiles.push(['Drivetrain', esc(l.drivetrain)]);
  if(l.transmission)                 tiles.push(['Transmission', esc(l.transmission)]);
  if(l.fuel_type)                    tiles.push(['Fuel', esc(l.fuel_type)]);
  if(s.fuel_consumption_l100)        tiles.push(['Consumption', esc(s.fuel_consumption_l100)+' \u2113/100km']);
  if(s.engine_capacity_cc)           tiles.push(['Engine', Number(s.engine_capacity_cc).toLocaleString('en-ZA')+' cc']);
  if(!tiles.length) return '';
  return '<div class="veh-qspec" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;">'
    + tiles.slice(0,6).map(t=>'<div style="background:var(--surface-2);border:1px solid var(--border);border-radius:10px;padding:8px 10px;text-align:center;"><div style="font-size:10px;color:var(--text-3);letter-spacing:.4px;text-transform:uppercase;">'+t[0]+'</div><div style="font-size:13px;font-weight:700;color:var(--text);margin-top:2px;">'+t[1]+'</div></div>').join('')
    + '</div>';
}
function vehSpecPanel(l){
  if(!vehHasSpecData(l)) return '';
  const s=vehSpecs(l);
  const val=(k)=>{ const c=l[k]; if(c!==undefined&&c!==null&&c!=='') return c; const j=s[k]; return (j!==undefined&&j!==null&&j!=='')?j:null; };
  const SEC=[
    ['Details',     ['make','model','variant','vehicle_year','colour','body_type','seats','doors']],
    ['Performance', ['engine_capacity_cc','kilowatts_kw','cylinder_layout','cylinders','aspiration','fuel_type','fuel_consumption_l100','fuel_tank_l','transmission','gears','drivetrain','tyre_front','tyre_rear','wheelbase_mm','co2_gkm']],
    ['Condition',   ['mileage_km','service_history','roadworthy_status','spare_key','maintenance_plan_until','warranty_until','condition_notes']],
    ['Features',    ['features']]
  ];
  const cards=SEC.map(([label,keys])=>{
    const rows=keys.map(k=>{
      let v=val(k); if(v===null) return '';
      if(Array.isArray(v)) v=v.join(', ');
      v=String(v).replace(/</g,'&lt;');
      return '<div style="display:flex;justify-content:space-between;gap:8px;font-size:12px;padding:3px 0;border-bottom:1px dashed var(--border);"><span style="color:var(--text-3);">'+(VEHICLE_SPEC_LABELS[k]||k)+'</span><span style="color:var(--text);font-weight:600;text-align:right;">'+v+'</span></div>';
    }).filter(Boolean).join('');
    if(!rows) return '';
    return '<div style="background:var(--surface-2);border:1px solid var(--border);border-radius:12px;padding:12px;"><div style="font-size:12px;font-weight:800;color:var(--text);margin-bottom:6px;">'+label+'</div>'+rows+'</div>';
  }).filter(Boolean);
  if(!cards.length) return '';
  const attest = l.attested_at ? '<div style="font-size:11px;color:var(--text-3);margin-top:8px;">\u2705 Seller-confirmed specifications \u00b7 attested '+String(l.attested_at).slice(0,10)+'</div>' : '';
...
async function signInSeller(){ return requestSignInLink('ob-signin-email','ob-signin-msg'); }

// One email box, no password — emails a magic sign-in link (register + login in one).
async function requestSignInLink(inputId, msgId){
  const emailEl = document.getElementById(inputId);
  const msg = document.getElementById(msgId);
  const email = ((emailEl && emailEl.value) || '').trim();
  if(msg) msg.style.display = 'block';
  if(!email){ if(msg){ msg.style.color='#ef4444'; msg.textContent='Enter your email first.'; } else showToast('Enter your email first'); return; }
  if(msg){ msg.style.color='var(--text-3)'; msg.textContent='Sending your link…'; }
  try{
...
// Home-header button = 'Sign in' when signed out, 'My Hub' shortcut when signed
// in. "Signed in" simply means an ms_aa_email is in localStorage. signOut()
// clears that identity so the button returns to a real sign-in screen.
function _msIsSignedIn(){ return !!((localStorage.getItem('ms_aa_email')||'').trim()); }
...
window.addEventListener('DOMContentLoaded', updateHeaderAuthBtn);

// ── ADD TO HOME SCREEN (PWA install) ───────────────────────────
let _deferredInstallPrompt = null;
window.addEventListener('beforeinstallprompt', function(e){ e.preventDefault(); _deferredInstallPrompt = e; });
function _isStandalone(){ try{ return window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true; }catch(e){ return false; } }
async function promptAddToHomeScreen(){
  if(_isStandalone()) return;
  if(localStorage.getItem('ms_a2hs_done')==='1') return;
  if(_deferredInstallPrompt){
    try{ _deferredInstallPrompt.prompt(); const c = await _deferredInstallPrompt.userChoice; _deferredInstallPrompt = null; if(c && c.outcome==='accepted') localStorage.setItem('ms_a2hs_done','1'); }catch(e){}
    return;
  }
  if(/iphone|ipad|ipod/i.test(navigator.userAgent)){
    showToast('Add TrustSquare to your home screen: tap Share then \"Add to Home Screen\".', 6000);
    localStorage.setItem('ms_a2hs_done','1');
  }
}

// ── DEMO MODE HELPERS ───────────────────────────────────────
// ── OPERATOR SKIN (ANON-TOURS companion, 7 Jul 2026): same console, tour-operator
// language. window._tsOperatorMode is set by the ?operator=1 deep-link; _agL()
// swaps labels only — same endpoints, same data. Agencies see the original text.
window._tsOperatorMode = window._tsOperatorMode || false;
function _agL(key){
...
async function openAgencyConsole(aidOverride, forceNew){
  ...
  // admin email still owns an estate test agency (by-admin returns first-only).
  if(aidOverride){ window._agencyId=aidOverride; await _renderAgency(aidOverride); return; }
  if(forceNew){ el.innerHTML=_agencyCreateHtml(isSuper,email); return; }
  let agencyId=null;
  if(email){
    try{ const r=await fetch(BEA_URL+'/agencies/by-admin/'+encodeURIComponent(email),{headers:{'X-Api-Key':API_KEY}}); if(r.ok){const d=await r.json(); agencyId=d.agency_id;} }catch(e){}
  }
  // Superuser fallback (7 Jul 2026): a superuser who admins no org of their own
  // (e.g. the testers - both orgs belong to David) lands on the org LIST, not the
  // create screen. Create screen only remains for superusers when zero orgs exist.
  if(!agencyId && isSuper){
    var _lst=await _agencyOrgList();
    if(_lst && _lst.length){ agencyId=_lst[0].id; }
  }
...
async function agencyCreate(){
  const name=(document.getElementById('ag-new-name')||{}).value||'';
  const admin=(document.getElementById('ag-new-admin')||{}).value||'';
  const cs=((document.getElementById('ag-new-countries')||{}).value||'').split(',').map(function(s){return s.trim();}).filter(Boolean);
  if(!name.trim()||admin.indexOf('@')<0){ showToast('Name and admin email required'); return; }
  try{ const r=await fetch(BEA_URL+'/agencies',{method:'POST',headers:{'Content-Type':'application/json','X-Api-Key':API_KEY},body:JSON.stringify({name:name,admin_email:admin,countries:cs})});
    const d=await r.json(); if(!r.ok){ showToast(d.detail||'Could not create'); return; }
    showToast(_agL('created')); window._agencyId=d.id; _renderAgency(d.id);
  }catch(e){ showToast('Could not reach server'); }
}
async function _renderAgency(agencyId){
  const el=document.getElementById('agency-body'); if(!el) return;
  el.innerHTML='<div style="padding:30px;text-align:center;color:var(--text-3);">Loading agency…</div>';
  let a=null;
  ...
  const _agRowFn=function(m){
    ...
    return '<tr style="border-bottom:1px solid #f1f3f7;">'
      ...
      +'<td style="padding:10px;">'+m.listings_live+' / '+cap+' <span style="display:inline-block;width:54px;height:6px;background:#eef1f6;border-radius:6px;vertical-align:middle;margin-left:6px;"><i style="display:block;height:100%;width:'+pct+'%;background:#C8873A;border-radius:6px;"></i></span></td>'
  ...
  if(localStorage.getItem('ms_superuser')==='1'){
    if(!window._tsOrgList){ _agencyOrgList().then(function(){ if(window._agencyId===agencyId) _renderAgency(agencyId); }); }
    var _opts=(window._tsOrgList||[]).map(function(o){ return '<option value="'+o.id+'"'+(o.id===a.id?' selected':'')+'>'+((o.name||('#'+o.id)).replace(/</g,'&lt;'))+'</option>'; }).join('');
    opsBar='<div style="display:flex;gap:8px;align-items:center;background:var(--surface,#fff);border:1px dashed var(--border);border-radius:12px;padding:10px 12px;margin-bottom:12px;flex-wrap:wrap;">'
  ...
  el.innerHTML= opsBar
    ...
    +'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px;">'
      ...
      +'<div style="background:var(--surface,#fff);border:1px solid var(--border);border-radius:12px;padding:12px;"><div style="font-size:22px;font-weight:800;font-family:Syne,sans-serif;">'+a.rollup.listings_used+' / '+a.rollup.listings_allowance+'</div><div style="font-size:11px;color:var(--text-3);">'+_agL('stock')+'</div></div>'
...
async function agencySeat(email,paid){
  try{ const r=await fetch(BEA_URL+'/agencies/'+window._agencyId+'/agents/'+encodeURIComponent(email),{method:'PUT',headers:{'Content-Type':'application/json','X-Api-Key':API_KEY},body:JSON.stringify({seat_paid:!!paid})});
    if(!r.ok){ showToast('Could not update seat'); return; } _renderAgency(window._agencyId);
  }catch(e){ showToast('Could not reach server'); }
}
async function agencyRemove(email){
  if(!confirm('Remove '+email+' from the '+_agL('org')+'?')) return;
  try{ const r=await fetch(BEA_URL+'/agencies/'+window._agencyId+'/agents/'+encodeURIComponent(email),{method:'DELETE',headers:{'X-Api-Key':API_KEY}});
    if(!r.ok){ showToast('Could not remove'); return; } _renderAgency(window._agencyId);
  }catch(e){ showToast('Could not reach server'); }
}
async function agencyRename(){
  if (typeof _demoBlock==='function' && _demoBlock('agencyRename')) return;
  var name=prompt('Rename "'+(window._agencyName||('this '+_agL('org')))+'" (#'+window._agencyId+') to:',''); if(!name||!name.trim()) return;
  try{ const r=await fetch(BEA_URL+'/agencies/'+window._agencyId,{method:'PUT',headers:{'Content-Type':'application/json','X-Api-Key':API_KEY},body:JSON.stringify({name:name.trim()})});
    const d=await r.json(); if(!r.ok){ showToast(d.detail||'Could not rename'); return; }
    showToast('Renamed to '+d.name); _renderAgency(window._agencyId);
  }catch(e){ showToast('Could not reach server'); }
}
async function _agencyOrgList(){
  if (typeof DEMO_MODE!=='undefined' && DEMO_MODE) return [];
  if(window._tsOrgList) return window._tsOrgList;
  ...
  _SUB_TIERS.forEach(t => {
    ...
    card.innerHTML = `
      ...
      <div style="display:flex;align-items:baseline;gap:4px;">
        <span style="font-size:20px;font-weight:800;color:var(--text);">${t.id==='agency'?'By application':(t.usd===0?'Free':'$'+t.usd)}</span>
...
// Mirrors the BEA derived label so demo mode (no server label) works client-side.
// Property listings only. kind: now | upc | occ | rsv.
function rentalAvail(l){
  if(!l || (l.cat||'')!=='Property') return null;
  var lab = l.availability_label || null;
  if(lab){
    var low = lab.toLowerCase();
    var k = low.indexOf('occupied')===0 ? 'occ' : (low.indexOf('under application')>-1 ? 'rsv' : (low.indexOf('available from')===0 ? 'upc' : 'now'));
    return {label: lab, kind: k};
  }
  var rs = (l.rental_status||'available').toLowerCase();
  var af = l.available_from ? String(l.available_from).slice(0,10) : '';
  var today = new Date().toISOString().slice(0,10);
  var up = !!af && af > today;
  if(rs==='occupied') return {label: up ? ('Occupied — available '+af) : 'Occupied', kind:'occ'};
  if(rs==='reserved') return {label:'Under application', kind:'rsv'};
  return {label: up ? ('Available from '+af) : 'Available now', kind: up?'upc':'now'};
}
// Card: badge only the states a browsing renter needs flagged (not the default 'Available now').
...
// Live mode: asks the BEA's FTS index (matches title/description/make/model/
// variant/subject/service-type, prefix-tolerant) and narrows the grid to the
// returned ids. Demo mode: plain local text match (demo rows live client-side).
let _msSearchQ = '', _msSearchTimer = null, _msSearchIds = null;

// Deterministic sentence interpreter (no AI): "Rental in Pretoria below R20000 per month"
// -> { price_max:20000, listingType:'rent', terms:[] }. Price phrases and rent/sale intent
// become structured dials; filler words and (typo-tolerant) city names are stripped;
// whatever remains goes to the FTS as keywords.
function _msLev(a, b){
  if (a === b) return 0;
...
function msParseQuery(raw){
  let t = ' ' + String(raw).toLowerCase().replace(/,/g, '') + ' ';
  const out = { price_min: null, price_max: null, listingType: null, terms: [] };
  const num = x => { const v = parseFloat(String(x).replace(/[\s]/g, '')); return isNaN(v) ? null : v; };
  t = t.replace(/\b(?:below|under|max(?:imum)?|up\s+to|less\s+than|cheaper\s+than)\s*r?\s*([\d][\d\s]*(?:\.\d+)?)\s*(k\b)?/g,
    (m, n, k) => { let v = num(n); if (v != null){ if (k) v *= 1000; out.price_max = v; } return ' '; });
  t = t.replace(/\b(?:above|over|min(?:imum)?|more\s+than|starting\s+from)\s*r?\s*([\d][\d\s]*(?:\.\d+)?)\s*(k\b)?/g,
    (m, n, k) => { let v = num(n); if (v != null){ if (k) v *= 1000; out.price_min = v; } return ' '; });
  // A bare budget ("R20 000", "20k") reads as a ceiling — buyers state maximums.
  t = t.replace(/\br\s?([\d][\d\s]*(?:\.\d+)?)\s*(k\b)?/g,
    (m, n, k) => { let v = num(n); if (v != null && out.price_max == null){ if (k) v *= 1000; out.price_max = v; } return ' '; });
  if (/\b(rentals?|rented|renting|to\s+rent|to\s+let|rent)\b/.test(t)){
    out.listingType = 'rent'; t = t.replace(/\b(rentals?|rented|renting|to\s+rent|to\s+let|rent)\b/g, ' ');
  } else if (/\b(for\s+sale|to\s+buy|buying|buy)\b/.test(t)){
    out.listingType = 'sale'; t = t.replace(/\b(for\s+sale|to\s+buy|buying|buy)\b/g, ' ');
  }
  const STOP = new Set(['in','for','a','an','the','with','per','pm','month','months','monthly','week','weekly',
    'me','my','near','around','close','of','and','or','to','below','under','above','over','price','priced',
    'budget','cheap','cheapest','looking','searching','wanted','want','need','please','r']);
  const cityName = ((typeof activeCity !== 'undefined' && activeCity && activeCity.name) || '').toLowerCase();
  out.terms = [];
  out.category = null;
  for (const w of t.split(/\s+/)){
    if (!w || STOP.has(w)) continue;
    if (cityName && w.length >= 5 && _msLev(w, cityName) <= 2) continue;  // typo-tolerant city strip
    if (/^\d+$/.test(w)){
      const v = parseInt(w, 10);
      if (v >= 1900 && v <= 2035){ out.terms.push(w); continue; }         // a year is a search term
      if (v >= 500 && out.price_max == null){ out.price_max = v; }        // a bare big number is a budget
      continue;                                                            // small bare numbers only hurt AND-matching
    }
    out.terms.push(w);
  }
  // Category evidence in the words themselves (kept as terms AND pinned as category —
  // so a relaxation pass can drop the words without losing WHERE the user meant).
  const CATMAP = {
  ...
  let _hit = null, _multi = false;
  for (const w of out.terms){
    for (const cat in CATMAP){
      if (CATMAP[cat].indexOf(w) !== -1){
        if (_hit && _hit !== cat) _multi = true;
        _hit = cat;
      }
    }
  }
  if (_hit && !_multi) out.category = _hit;
  if (out.listingType && !out.category) out.category = 'Property';   // rent/sale talk = property talk
  return out;
}
// SEARCH-HMI-1: search-from-anywhere routing — typed intent beats the active chip.
// If the current context would hide every match, jump to where the matches are.
...
function msRouteToMatches(rows, parsedCategory, explicit){
  ...
  // The user's WORDS outrank match-counting: "apartment" pins Property even when
  // cheap Collectors cards dominate the price window.
  const intent = (parsedCategory && (counts[parsedCategory]||0) > 0) ? parsedCategory : null;
  const onAdv = !!document.querySelector('#screen-adventures.active');
  if (onAdv){
    if ((counts['Adventures']||0) > 0) return;   // matches right here — stay put
    if (!explicit) return;                        // never yank screens mid-keystroke — Enter routes
    goTo('browse');
    _msApplyChip(intent || (topShare >= 0.7 ? top : 'All'));
    return;
  }
  if (intent && activeFilter !== intent && activeFilter !== 'All'){
    _msApplyChip(intent);                         // words said where to go — follow them
    return;
  }
  const visible = activeFilter === 'All' ? rows.length : (counts[activeFilter]||0);
  if (visible > 0) return;                        // current chip already shows matches — don't yank
  // Adventures matches render under the All chip in Browse; never bounce screens mid-search.
  _msApplyChip(intent || ((topShare >= 0.7 && top !== 'Adventures') ? top : 'All'));
}
function _msRerender(){
  renderGrid();
  if (document.querySelector('#screen-adventures.active') && typeof renderAdvGrid === 'function') renderAdvGrid();
}
// Adventures screen bar — mirrors into the one search engine, so a routed jump
// to Browse carries the query with it.
function advSearchInput(){
  const el = document.getElementById('adv-search-input');
  const mirror = document.getElementById('ms-search-input');
  if (mirror && el) mirror.value = el.value;
  msDebouncedSearch();
}
function advSearchEnter(el){
  const mirror = document.getElementById('ms-search-input');
  if (mirror && el) mirror.value = el.value;
...
async function msRunSearch(explicit){
  ...
  const parsed = msParseQuery(_msSearchQ);
  // A rent/sale intent in the sentence sets the Property dial (visible, removable).
  if (parsed.listingType && (activeFilter === 'Property' || activeFilter === 'All')){
    filterState.property.listingType = parsed.listingType;
    if (typeof renderActiveFilterTags === 'function') renderActiveFilterTags();
  }
  if (DEMO_MODE) {
    _msSearchIds = new Set(LISTINGS.filter(l => {
      const hay = ((l.title||'') + ' ' + (l.desc||l.description||'')).toLowerCase();
      if (parsed.terms.length && !parsed.terms.every(w => hay.includes(w))) return false;
      const p = l.priceNum || 0;
      if (parsed.price_max != null && p > parsed.price_max) return false;
      if (parsed.price_min != null && p < parsed.price_min) return false;
      return true;
    }).map(l => String(l.id)));
  } else {
    const base = '/listings?city=' + encodeURIComponent(activeCity.name || '') + '&page_size=200' +
      ...
      try {
        ...
        if (ai && ai.enabled && ai.params){
          ...
          if (p.listingType && (p.category == null || p.category === 'Property') &&
              (activeFilter === 'Property' || activeFilter === 'All')){
            filterState.property.listingType = p.listingType;
            if (typeof renderActiveFilterTags === 'function') renderActiveFilterTags();
          }
          let u = '/listings?city=' + encodeURIComponent(activeCity.name || '') + '&page_size=200';
          if (p.category) u += '&category=' + encodeURIComponent(p.category);
          if (p.terms && p.terms.length) u += '&q=' + encodeURIComponent(p.terms.join(' '));
  ...
  if (_msSearchQ.length >= 2 && typeof wlCaptureSearch === 'function') {
    try { wlCaptureSearch(_msSearchQ, activeFilter === 'All' ? null : activeFilter,
                          _msSearchIds ? _msSearchIds.size : null); } catch(e) {}
  }
  _msRerender();
}
...
function cvAvatarHtml(sellerIdx, listingId, isOwner){
  ...
  const s = (SELLERS.find(x=>x.idx===idx))||SELLERS[idx]||SELLERS[0];
...
function openCVEdit(){
  // CV-EDIT-FIX-1 (20 Jul 2026, David-found): live site has SELLERS=[] (demo-era
  // dependency) so SELLERS[0] threw and the button died silently. Load the saved
  // profile from localStorage, else a clean skeleton.
  var base = (typeof SELLERS!=='undefined' && SELLERS && SELLERS[0]) ? SELLERS[0] : null;
  if(!base){ try{ base = JSON.parse(localStorage.getItem('ms_seller_profile')||'null'); }catch(e){ base=null; } }
  if(!base) base = { headline:'', about:'', yearsExp:'', region:(typeof activeCity!=='undefined'&&activeCity.name)||'',
                     tags:[], creds:[], avail:[] };
  editingSeller = JSON.parse(JSON.stringify(base));
  editingSeller.headline = editingSeller.headline||''; editingSeller.about = editingSeller.about||'';
  editingSeller.tags = editingSeller.tags||[]; editingSeller.creds = editingSeller.creds||[];
  editingSeller.avail = editingSeller.avail||[];
...
function saveCVEdit(){
  ...
  // Write back to SELLERS array and persist (CV-EDIT-FIX-1: SELLERS may be empty on live)
...
function sobGoPhase(n) {
  ...
  // CARS-SPEC-1: render the vehicle section-confirm cards (no-op for non-cars)
  if (n === 3) { try { sobRenderAttest(); } catch(e) {} }
...
// ── CARS-SPEC-1 · Vehicle section-confirm + attestation (sob phase 3) ──────
// Mirror of BEA VEHICLE_SECTION_FIELDS — section-level confirmation ONLY
// (locked decision: never per-field ticks). Values stay hidden from buyers
// until section-confirm + attestation (BEA scrub enforces server-side).
const VEHICLE_SECTIONS_JS = {
  details:     { label:'Details',     cols:['make','model','variant','vehicle_year','colour','body_type'], keys:['seats','doors'] },
  performance: { label:'Performance', cols:['transmission','fuel_type','drivetrain'], keys:['engine_capacity_cc','kilowatts_kw','cylinder_layout','cylinders','aspiration','fuel_consumption_l100','fuel_tank_l','gears','tyre_front','tyre_rear','wheelbase_mm','co2_gkm'] },
  condition:   { label:'Condition',   cols:['mileage_km'], keys:['service_history','roadworthy_status','spare_key','maintenance_plan_until','warranty_until','condition_notes'] },
  features:    { label:'Features',    cols:[], keys:['features'] }
};
const VEHICLE_SPEC_LABELS = { make:'Make', model:'Model', variant:'Variant', vehicle_year:'Year', colour:'Colour', body_type:'Body type', seats:'Seats', doors:'Doors', transmission:'Transmission', fuel_type:'Fuel type', drivetrain:'Drivetrain', engine_capacity_cc:'Engine (cc)', kilowatts_kw:'Power (kW)', cylinder_layout:'Cylinder layout', cylinders:'Cylinders', aspiration:'Aspiration', fuel_consumption_l100:'Consumption (\u2113/100km)', fuel_tank_l:'Tank (\u2113)', gears:'Gears', tyre_front:'Tyre front', tyre_rear:'Tyre rear', wheelbase_mm:'Wheelbase (mm)', co2_gkm:'CO\u2082 (g/km)', mileage_km:'Mileage (km)', service_history:'Service history', roadworthy_status:'Roadworthy', spare_key:'Spare key', maintenance_plan_until:'Maintenance plan until', warranty_until:'Warranty until', condition_notes:'Condition notes', features:'Features' };

function sobVehicleData(){
  // Vehicle data for the first cars draft: guided stash first (goHandoff path),
  // else the draft row itself (sobInit /listings/mine path returns all columns).
  const draft=(sobState.drafts||[]).find(d=>(d.category||'').toLowerCase()==='cars');
  if(!draft) return null;
  const src=(typeof goState!=='undefined' && goState.vehicle && sobState._cameFromGuided) ? goState.vehicle : draft;
  let specs={};
  try{ specs=JSON.parse(src.vehicle_specs||'{}')||{}; }catch(e){ specs={}; }
  const prov=(specs&&specs._prov)||{};
  const out={ draftId:draft.id, sections:{}, prov:prov };
  Object.keys(VEHICLE_SECTIONS_JS).forEach(sec=>{
    const def=VEHICLE_SECTIONS_JS[sec]; const rows=[];
    def.cols.forEach(k=>{ const v=src[k]; if(v!==undefined&&v!==null&&v!=='') rows.push([k,v]); });
    def.keys.forEach(k=>{ const v=specs[k]; if(v!==undefined&&v!==null&&v!=='') rows.push([k,Array.isArray(v)?v.join(', '):v]); });
    if(rows.length) out.sections[sec]=rows;
  });
  return Object.keys(out.sections).length?out:null;
}

function sobRenderAttest(){
  const wrap=document.getElementById('sob-attest-wrap');
  const host=document.getElementById('sob-attest-sections');
  if(!wrap||!host) return;
  let data=null;
  try{ data=sobVehicleData(); }catch(e){ data=null; }
...
function sobConfirmSection(sec){
  if(!sobState._attest) return;
  sobState._attest.confirmed[sec]=new Date().toISOString().slice(0,19);
  const btn=document.getElementById('sob-attest-btn-'+sec);
  if(btn){ btn.textContent='Confirmed \u2713'; btn.style.background='rgba(134,239,172,.15)'; btn.style.color='#86efac'; btn.style.borderColor='rgba(134,239,172,.4)'; btn.disabled=true; }
  sobCheckEula();
}

function sobAttestSatisfied(){
  const a=sobState._attest;
  if(!a) return true;                          // no vehicle data → legacy behaviour
  const chk=document.getElementById('sob-attest-chk');
...
async function sobGoLive() {
  ...
  for (const draft of sobState.drafts) {
    try {
      ...
      // 2. CARS-SPEC-1: persist section confirmations, then publish attested
      let _attQ = '';
      if (sobState._attest && sobState._attest.draftId === draft.id && sobAttestSatisfied()) {
        try {
          await fetch(BEA_URL + '/listings/' + draft.id + '?email=' + encodeURIComponent(email), {
            method: 'PUT', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ spec_confirmed: JSON.stringify(sobState._attest.confirmed) })
          });
        } catch(e) {}
        _attQ = '&attested=1';
      }
...
function goRevealDraft(draft, warnings, anonymityScrubbed, violatingIndices) {
  ...
  // Advert Coach — soft template tips (dealer/portal/tour-operator formats). Never blocking.
  try {
    let tipsEl = document.getElementById('go-coach-tips');
    if (!tipsEl && shotsEl) {
      tipsEl = document.createElement('div');
      tipsEl.id = 'go-coach-tips';
      shotsEl.insertAdjacentElement('afterend', tipsEl);
    }
    const _tips = (draft.coach_tips || []).filter(t => typeof t === 'string' && t.trim()).slice(0, 4);
    if (tipsEl) {
      if (_tips.length) {
  ...
  } catch(e) { console.warn('coach tips render skipped', e); }
...
function goApplyVisionToStep2() {
  ...
  // CARS-SPEC-1: stash the AI-drafted vehicle identity + spec sheet on goState.
  // Provenance ai_guess — the seller has NOT confirmed these; the BEA scrub keeps
  // them off public reads until section-confirm + attestation (Task 4 UI).
  if ((goState.cat||'').toLowerCase()==='cars') {
    const v = {};
    ...
    const specs = (d.vehicle_specs && typeof d.vehicle_specs === 'object' && !Array.isArray(d.vehicle_specs)) ? {...d.vehicle_specs} : {};
    delete specs._prov;
    const prov = {};
    Object.keys(v).forEach(k => { prov[k] = 'ai_guess'; });
    Object.keys(specs).forEach(k => { prov[k] = 'ai_guess'; });
    if (Object.keys(prov).length) specs._prov = prov;
    if (Object.keys(specs).length && Object.keys(specs).some(k=>k!=='_prov')) v.vehicle_specs = JSON.stringify(specs);
    else if (Object.keys(prov).length) v.vehicle_specs = JSON.stringify({_prov: prov});
    if (Object.keys(v).length) goState.vehicle = v;
  }
...
async function goHandoff() {
  ...
  // ── Tentative-seller safety net (save = magic link) ──────────────
...
async function sbSaveDraftAndExit(){
  // PREVENT-PUBLISH: go-live-before-exit guard (GUIDED-PUBLISH-1 follow-up).
...
const SB_FIELDS = {
  ...
  Cars: [
    ...
    {id:'variant',       label:'Variant / trim',    type:'text',   placeholder:'e.g. 2.8 GD-6 4x4 Legend'},
...
// CARS-SPEC-1: map quick-list Cars fields → structured vehicle columns.
// Seller typed every value here → provenance seller_entered
// (mirror of BEA VEHICLE_SECTION_FIELDS provenance convention).
function sbVehicleBody(f){
  const b={};
  if(f.make)    b.make=f.make;
  if(f.model)   b.model=f.model;
  if(f.variant) b.variant=f.variant;
  if(f.year && parseInt(f.year,10))       b.vehicle_year=parseInt(f.year,10);
  if(f.mileage && parseInt(f.mileage,10)) b.mileage_km=parseInt(f.mileage,10);
  if(f.transmission) b.transmission=f.transmission;
  if(f.colour)  b.colour=f.colour;
  const specs={}, prov={};
  Object.keys(b).forEach(k=>{ prov[k]='seller_entered'; });
  // Engine text → engine_capacity_cc ONLY for explicit cc numbers (never derived from litres — no guessing)
  const cc=parseInt(String(f.engine||'').replace(/[^0-9]/g,''),10);
  if(cc>=600&&cc<=9000){ specs.engine_capacity_cc=cc; prov.engine_capacity_cc='seller_entered'; }
  if(Object.keys(prov).length) specs._prov=prov;
  if(Object.keys(specs).length) b.vehicle_specs=JSON.stringify(specs);
  return b;
}
...
async function sbHandleDocUpload(e){
  ...
  sbState.signals[signalId]={status:'uploaded', pts:sig.pts, file:file};
  if(feedbackEl){
    feedbackEl.style.background='rgba(22,163,74,.08)';
    feedbackEl.style.color='#16a34a';
    feedbackEl.textContent=`✓ Attached — saves to your Trust Score when you publish.`;
  }
  sbUpdateB7Score();
...
async function sbDoPublish(){
  ...
  try {
    ...
    if(BEA_ENABLED){
      ...
      const _veh = sbState.cat==='Cars' ? sbVehicleBody(f) : {};   // CARS-SPEC-1
    ...
    if(BEA_ENABLED && email && sbState.signals){
      for(const _sigId in sbState.signals){
        const _s=sbState.signals[_sigId];
        if(!_s || !_s.file || _s.status!=='uploaded') continue;
        try{
          const _dfd=new FormData();
          _dfd.append('file',_s.file);
          _dfd.append('doc_type','other');
          _dfd.append('signal_id','category.'+_sigId);
          const _sig=(sbGetSignalSet().find(x=>x.id===_sigId)||{});
          _dfd.append('label',_sig.label||_sigId);
          _dfd.append('visibility','private');
          await fetch(BEA_URL+'/users/'+encodeURIComponent(email)+'/documents',{method:'POST',headers:{'X-Api-Key':API_KEY},body:_dfd}).catch(()=>{});
        }catch(_){}
      }
    }
...
function sbLifecycleChip(ls){
  var s=(ls||'live').toLowerCase();
  var M={
...
function _elGetFields(cat, serviceClass) {
  ...
  if (!catDef) return [];
  if (catDef.serviceClasses) {
    const sc = serviceClass || catDef.defaultClass || Object.keys(catDef.serviceClasses)[0];
    return (((catDef.serviceClasses[sc] || catDef.serviceClasses[catDef.defaultClass]) || {}).fields) || catDef.fields || [];
  }
...
const AA_CATEGORIES = {
  ...
  'Services': {
    ...
    defaultClass: 'Technical',
    classHint: 'Choose Technical for licensed trades, or Casuals for domestic &amp; informal work',
    serviceClasses: {
      'Technical': {
        label: '🔧 Technical',
        fields: [
  ...
  'Adventures': {
    ...
    defaultClass: 'adventures_accommodation',
    classLabel: 'Listing type',
    serviceClasses: {
      'adventures_accommodation': {
        label: '🏕 Accommodation',
        fields: [
  ...
  'Cars': {
    emoji: '🚗',
    // Vehicle sub-types (VEHICLES-SUBTYPE-1, brief B0) — internal category id stays 'Cars'.
    // Top-level fields/photos = the Car set, kept for consumers that read .fields directly.
    defaultClass: 'Car',
    classLabel: 'What are you selling?',
    classHint: 'Pick the closest type — the form adapts to it',
    serviceClasses: {
      'Car': {
        label: '🚗 Car / bakkie / SUV',
        fields: [
          ...
          {id:'variant',     label:'Variant / trim (optional)', type:'text', required:false, placeholder:'e.g. 4x4 Legend, 2.0 TDI Highline', helpText:'The trim line — buyers filter by it'},
          {id:'year',        label:'Year',                   type:'number', placeholder:'e.g. 2019'},
          {id:'mileage',     label:'Mileage (km)',            type:'number', placeholder:'e.g. 85000'},
          {id:'colour',      label:'Colour',                 type:'text',   placeholder:'e.g. White'},
        ...
        photos: [
          ...
          {slot:'odometer',       label:'Odometer (optional)',  hint:'Current mileage clearly visible',        required:false},
      ...
      'Bike': {
        label: '🏍 Motorcycle',
        fields: [
          {id:'title',       label:'Listing headline',  type:'text',   placeholder:'e.g. 2021 BMW R1250 GS · low km'},
          {id:'make',        label:'Make',              type:'text',   placeholder:'e.g. BMW'},
          {id:'model',       label:'Model',             type:'text',   placeholder:'e.g. R1250 GS'},
          {id:'year',        label:'Year',              type:'number', placeholder:'e.g. 2021'},
          {id:'cc',          label:'Engine (cc)',       type:'number', placeholder:'e.g. 1250'},
          {id:'mileage',     label:'Mileage (km)',       type:'number', placeholder:'e.g. 18000'},
          {id:'bike_type',   label:'Bike type',         type:'select', options:['Adventure','Road / Sport','Cruiser','Off-road / MX','Scooter / Commuter','Classic','Quad / ATV','Other']},
          {id:'colour',      label:'Colour',            type:'text',   placeholder:'e.g. Black'},
          {id:'price',       label:'Asking price',      type:'number', placeholder:'e.g. 165000'},
          {id:'listing_type',label:'Listing type',      type:'select', options:['For Sale','For Hire / Rental']},
          {id:'desc',        label:'Description',       type:'textarea',placeholder:'Condition, service history, extras, accessories…'},
        ],
        photos: [
          {slot:'right_side',   label:'Right side',            hint:'Full profile, stand up if possible', required:true},
          {slot:'left_side',    label:'Left side',             hint:'Full profile',                       required:true},
          {slot:'odometer',     label:'Odometer / dash',       hint:'Mileage clearly visible',            required:false},
          {slot:'service_book', label:'Service book (optional)',hint:'Open to last service entry',        required:false},
        ],
      },
      'Caravan': {
        ...
        fields: [
          {id:'title',        label:'Listing headline',   type:'text',   placeholder:'e.g. 2018 Jurgens Penta · neat'},
          {id:'trailer_type', label:'Type',               type:'select', options:['Caravan','Camper / off-road trailer','Utility trailer','Horsebox','Boat trailer','Motorhome','Other']},
          {id:'make',         label:'Make',               type:'text',   placeholder:'e.g. Jurgens'},
          {id:'model',        label:'Model',              type:'text',   placeholder:'e.g. Penta'},
          {id:'year',         label:'Year',               type:'number', placeholder:'e.g. 2018'},
          {id:'sleeps',       label:'Sleeps (caravans)',  type:'number', required:false, placeholder:'e.g. 4'},
          {id:'braked',       label:'Braked axle',        type:'select', options:['Yes','No','Not sure']},
          {id:'papers',       label:'Registration papers',type:'select', options:['Registered with papers','Unregistered','Papers in progress']},
          {id:'price',        label:'Asking price',       type:'number', placeholder:'e.g. 120000'},
          {id:'listing_type', label:'Listing type',       type:'select', options:['For Sale','For Hire / Rental']},
          {id:'desc',         label:'Description',        type:'textarea',placeholder:'Condition, layout, extras (awning, solar), service history…'},
        ],
        photos: [
          {slot:'exterior_side', label:'Exterior — side',   hint:'Full profile, hitched or level',   required:true},
          {slot:'interior',      label:'Interior (caravans)',hint:'Main living/sleeping area',        required:false},
          {slot:'coupling',      label:'Coupling / towbar',  hint:'Hitch, jockey wheel, chains',      required:false},
          {slot:'papers_photo',  label:'Papers (optional)',  hint:'Registration document',            required:false},
        ],
      },
      'Boat': {
        label: '🛥 Boat & watercraft',
        fields: [
          {id:'title',            label:'Listing headline', type:'text',   placeholder:'e.g. 2016 Raptor 660 · Yamaha 115'},
          {id:'boat_type',        label:'Boat type',        type:'select', options:['Bass / fishing','Ski / wake','Cabin / cruiser','Rubber duck / inflatable','Jet ski','Sail','Canoe / kayak','Other']},
          {id:'make',             label:'Make',             type:'text',   placeholder:'e.g. Raptor'},
          {id:'model',            label:'Model',            type:'text',   placeholder:'e.g. 660 Fish'},
          {id:'year',             label:'Year',             type:'number', placeholder:'e.g. 2016'},
          {id:'engine_make',      label:'Engine',           type:'text',   placeholder:'e.g. Yamaha 115 4-stroke'},
          {id:'engine_hours',     label:'Engine hours',     type:'number', required:false, placeholder:'e.g. 240'},
          {id:'trailer_included', label:'Trailer included', type:'select', options:['Yes — registered','Yes — unregistered','No']},
          {id:'price',            label:'Asking price',     type:'number', placeholder:'e.g. 295000'},
          {id:'listing_type',     label:'Listing type',     type:'select', options:['For Sale','For Hire / Rental']},
          {id:'desc',             label:'Description',      type:'textarea',placeholder:'Condition, service history, electronics, safety gear…'},
        ],
        photos: [
          {slot:'hull_side',  label:'Hull — side profile', hint:'Full boat, on trailer or water', required:true},
          {slot:'helm',       label:'Helm / console',      hint:'Controls and electronics',       required:true},
          {slot:'engine',     label:'Engine',              hint:'Outboard or engine bay',         required:false},
          {slot:'on_trailer', label:'On trailer (optional)',hint:'Shows the trailer condition',   required:false},
        ],
      },
      'Other': {
        label: '🚜 Truck · bus · tractor',
        fields: [
          {id:'title',        label:'Listing headline',  type:'text',   placeholder:'e.g. 2015 Hino 500 · 8 ton'},
          {id:'vehicle_type', label:'Vehicle type',      type:'select', options:['Truck','Bus / minibus','Tractor','Forklift','Earthmoving','Other commercial']},
          {id:'make',         label:'Make',              type:'text',   placeholder:'e.g. Hino'},
          {id:'model',        label:'Model',             type:'text',   placeholder:'e.g. 500 916'},
          {id:'year',         label:'Year',              type:'number', placeholder:'e.g. 2015'},
          {id:'mileage',      label:'Km / hours',        type:'number', placeholder:'e.g. 320000'},
          {id:'capacity',     label:'Capacity',          type:'text',   required:false, placeholder:'e.g. 8 ton / 22 seats / 90 kW'},
          {id:'price',        label:'Asking price',      type:'number', placeholder:'e.g. 450000'},
          {id:'listing_type', label:'Listing type',      type:'select', options:['For Sale','For Hire / Rental']},
          {id:'desc',         label:'Description',       type:'textarea',placeholder:'Condition, service history, licences, load/PTO details…'},
        ],
        photos: [
          {slot:'front',    label:'Front',                hint:'Head-on, full vehicle',      required:true},
          {slot:'side',     label:'Side',                 hint:'Full profile',               required:true},
          {slot:'cab',      label:'Cab interior',         hint:'Seats, dash, hour meter',    required:false},
          {slot:'engine',   label:'Engine (optional)',    hint:'Engine bay or PTO',          required:false},
        ],
      },
    },
    fields: [
...
function aaRenderDetailForm(category, values, serviceClass, suggestions) {
  ...
  // Class selector chips — generic: any category with serviceClasses (Services, Cars sub-types, Adventures)
  if (base.serviceClasses) {
    const _clsLabel = base.classLabel || 'Service Class';
    const _clsHint  = base.classHint || '';
    const classHtml = `
      <div class="aa-section-label">${_clsLabel}</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
        ${Object.keys(base.serviceClasses).map(k => `<button class="aa-cat-chip ${serviceClass===k?'selected':''}" onclick="aaSelectServiceClass('${k}')">${base.serviceClasses[k].label||k}</button>`).join('')}
      </div>
      ${serviceClass?'':(_clsHint?`<p style="color:var(--text-3);font-size:12px;margin-top:4px;">${_clsHint}</p>`:'')}
    `;
    if (!serviceClass) {
...
async function aaSaveDetailAndNext() {
  ...
  // Any category with serviceClasses (Services, Cars sub-types): class comes from the draft
  const existingDraft = await aaDB.get(aaDraftId) || {};
  const _catDef = AA_CATEGORIES[category] || {};
  const service_class = _catDef.serviceClasses ? (existingDraft.service_class || null) : null;
  if (_catDef.serviceClasses && !service_class) {
...
function wlCaptureSearch(query, cat, resultCount) {
  ...
  _wlSearchDebounce = setTimeout(() => {
    // result_count: 0 = a demand MISS — feeds the demand-loop (DEMAND-LOOP-1)
...
async function msSellerSignIn() {
  ...
  try {
    ...
    if (user && user.email) {
      ...
      try{ msInit(); }catch(e){}   // re-run My Space init so the OPS card reveals immediately after sign-in (7 Jul 2026)
...
function msInit(){
  ...
  if(opsCard && !(typeof isSuperuser==='function' && isSuperuser()) && !(typeof DEMO_MODE!=='undefined' && DEMO_MODE)){
    var _sueml=localStorage.getItem('ms_aa_email')||localStorage.getItem('ms_user_email')||'';
    if(_sueml){ (async function(){ try{ const u=await apiGet('/users/'+encodeURIComponent(_sueml));
      if(u && u.is_superuser){ localStorage.setItem('ms_superuser','1'); opsCard.style.display=''; }
    }catch(e){} })(); }
  }
...
function aiSel(id){
  ...
  try {
    if (localStorage.getItem('ms_dev') !== '1') {
      const _dc = document.getElementById('ai-dryrun');
      if (_dc) { _dc.checked = false; const _w = _dc.closest('label') || _dc.parentElement; if (_w) _w.style.display = 'none'; }
    }
  } catch(e) {}
...
// Gold chip linking to the delivered interactive route map (map_url from the AA
...
function aiPrint(){
  ...
  w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>'+AI_LAST.fn+' — TrustSquare report</title><style>'
   ...
   +'<h1>'+AI_LAST.fn+'</h1><div class="sub">TrustSquare AI report · '+new Date().toLocaleString()+' · all figures are estimates</div>'
   ...
   +'<div class="foot">Generated by TrustSquare AI Features · estimates and awareness only — not certified valuations, financial, legal or safety advice · trustsquare.co</div>'
...
async function aiExample(){
  ...
  try{
    ...
    const sm=txt.match(/##\s*\d*\s*[\u00b7]?\s*Safety awareness[\s\S]*?(?=\n##\s|$)/);
    if(sm){aiSafety(aiMd(sm[0].replace(/##\s*\d*\s*[\u00b7]?\s*/,'## \u26a0\ufe0f ')));txt=txt.replace(sm[0],'');}
    st.innerHTML='<span class="ai-ok">\u2713 Example \u2014 this is a sample of what you receive (free, no Tuppence used)</span>'+(d.map_url?aiMapBtn(d.map_url):'');
    txt = aiStripJson(txt);
...
async function aiListLot(){
  ...
  const draft = { id:'aa_'+Date.now(), email:aiSeedEmail(), category:'Collectors', stage:1,
    fields:{ title:lotTitle.slice(0,80), item_type:aiTypeMap(AI_ITEMS[0].item_type),
      condition:'Per item — AI visual estimates in report (final grades follow TrustSquare assessment)',
  ...
  for(let i=0;i<AI_ITEMS.length;i++){
    ...
    const draft = { id:'aa_'+(base+i), email:aiSeedEmail(), category:'Collectors', stage:1,
      fields:{ title:(it.title||'Item '+(i+1)).slice(0,80), item_type:aiTypeMap(it.item_type),
        condition:(it.condition_est||'')+' (AI visual estimate — final grade follows TrustSquare assessment)',
