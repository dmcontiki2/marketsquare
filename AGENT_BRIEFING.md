...
---
## Applicable article — BIT Agent / self-test (27 Jun 2026)
TrustSquare has a Built-In Test (BIT) self-test, governed by **Codex v4.8 §13** and specced in **BIT_ARCHITECTURE.md**. Hard rules for any agent:
- The BIT is a **SEPARATE, minimal article** (`../trustsquare-bit-agent/`), NEVER code inside `bea_main.py`. It shares no process/deploy/DB/deps with the app and talks to it read-only over HTTP. Keeping it out of the app is the point — an embedded self-test raises the app's own failure rate.
