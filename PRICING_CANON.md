...
### 2d · One $5 tier — Global folds INTO Starter (David, 14 Sep 2026 — RUL-128; built 17 Sep 2026, flag-dark)
**David chose the mechanism RUL-080 reserved to him: fold Global reach into Starter.** Once `launch_switches.baseline_q4` is armed (RUL-126, David's act) the ladder reads **Free (local, 2 slots) · Starter $5 (10 slots + global buyer reach) · Pro $20 (30 slots + reach + Squire)** and the separate buyer subscription retires: `POST /wishlist/subscription/initialize` answers 410 ("Global reach now comes with the Starter plan"), `GET /pricing/ladder` serves the one ladder the pricing page renders, and `_buyer_tier()` / `_buyer_reach_tier()` resolve a live **Starter** subscription to `global` exactly as they already resolve Pro (§2c). The table in §2 stays the pre-fold shape and stays TRUE until arming.

**Execution care — `wishlist_subscriptions` is a LIVE Paystack-backed table.** An existing Global subscriber keeps what they bought, at the same price, and is never cancelled and re-sold: on arming, `_fold_global_into_starter()` grants Starter (10 slots) to every account holding an unexpired Global row, audited per account in `admin_audit`, and touches neither the wishlist row (which still grants reach) nor Paystack. Idempotent. Probed 17 Sep 2026: the live table holds **0** rows, so the fold moves nobody today; the machinery exists for whoever buys before arming.

**Consequence David accepted in choosing it:** a pure buyer who never sells now buys a seller plan to get reach — same $5, plus slots they may not use. **Not ruled here:** the Agency Pro seat (RUL-048) stays its own $5 price point.
