...
- [LAUNCH-CONTROL-1 · build] The (+1) Launch Control view: per-city gate board (8 gates live-status), pool depth + wave→conversion funnel (CityLaunch data), KPI panel with both polarities, payment-rail health, David's halt/approve buttons (orchestrator /approve pattern). Builds on CityLaunch UI + ops dashboard plumbing.
