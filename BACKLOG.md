...
| DEMO-4 | Refresh ~6 dead demo image links (unsplash URLs returning 404): 4 listings have a dead single `photo`, 2 also have a dead `photos[0]`. The v149 graceful fallback now shows a category placeholder instead of a gray box, but the actual demo images should be replaced. Demo-only. | Data | ATTENDED |
