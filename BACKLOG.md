...
|-------|------|--------------|-------------|
| 12 Sep | **Quick Listing app — the next design track (RUL-124)** — tap-only harness, five taps to a draft, 4+3 to five results; work-seekers as a fourth Services sub-pick (head-hunter search, no CV store, company pays the intro, seeker never pays); ships in PARALLEL as its own app with its own coloured TrustSquare logo, same accounts/server/scores. Design board: Ops Dashboard → Quick Listing tab. Open design items: name the wider worker category; the hirer's side of the communication link; sub-path vs subdomain for the parallel app; the coloured logo asset; strip embedded photos from the working harness file. | Not yet live by ruling — goes live when David is happy with the whole and baseline readiness holds | Yes — nothing touches the live app until then |
| 2 Aug | **Counsel: lawyer-draft v1.9 "penalty halves every 90 days" contradicts canon decay rules** (was OPEN_LOOPS D2) — flag to counsel for reconciliation when counsel work next opens. | David 2 Aug: "remove as outstanding — I will re-open when the time is right." | Yes — reopen as a loop any time |
| 2 Aug | **Counsel: consolidate the docx vs web EULA forks into one lineage** (was OPEN_LOOPS D3; also tracked in LEGAL_VERSIONS.md as A6, which stays authoritative) — send both forks to counsel with the v1.11 web fork marked current. | David 2 Aug: same ruling as the decay-rules item — parked until counsel engagement. | Yes — reopen any time |
...
| 14 Aug | **Cars: ground the vehicle-spec lane, or keep it declared (TS-0031, David Jnr)** — today make/model/variant are inferred by VISION over the seller's photos plus a model prior (CARS-SPEC-1), and the market note is a one-sentence ungrounded Haiku. There is NO lookup in this lane, so an uncommon make or variant can come back confidently wrong — which is what the tester hit. The honest half shipped 14 Aug (SPEC-PROVENANCE-1: the attestation screen now says the values were read from photos, not looked up). What remains is David's call: (a) leave it declared and seller-attested as now, (b) ground it against a real vehicle-spec source (cost + a supplier that must not become load-bearing, per the fallback doctrine), or (c) drop the AI market note. **DECIDED 15 Aug (David): (a) — stays declared + seller-attested (RUL-016); TS-0031 closed, letter sent.** | Design/product decision + possible external data cost — not a mechanical fix | Closed 15 Aug |
| 15 Aug | **P0 LAUNCH GATE — LIST-VISIBILITY-1: machine-prove the core promise on every deploy** — BIT probe that creates a listing through the real API (demo-flagged), uploads a photo, then as a FRESH client confirms: listing in /listings AND photo URL serves 200, all within 60 seconds; then cleans up. Red = deploy auto-reverts. Why: David's 15 Aug question ('proof that we cannot list?') — the user upload lane is architecturally sound (no cache headers on /listings, FEA fetches on load, every upload gets a never-cached URL) but has only 17 photos of lifetime use; soundness must be MEASURED, not assumed. The half-day/overnight visibility pain was the curated/seeded lane (deploy-coupled + fixed-URL mutations), not the user lane — the probe proves that distinction stays true. | The launch-confidence gate David asked for | Yes |
| 15 Aug | **SO-2 parity audit of ALL exemplar photo sets** — David's representation-parity ruling (STANDING_ORDERS SO-2 / RUL-018) is now law for new renders; the existing ~100 listings' sets have not been audited against it. Sweep every set side-by-side (people visible? dress standard identical? anyone identifiable?), queue offending photos in HIGGSFIELD_REGEN_QUEUE.md as versioned re-cuts. First candidates: svccas photo 2 (visible forearm), svctech set wrist check. | Brand-safety, pre-launch priority — cheap to check, expensive to miss | Yes |
...
| 7 Sep | **BOTs — Bolted On Terminals, and the wish genie (RUL-108)** — a per-purpose home-screen icon opening a voice-first genie into the app. PROBED live 7 Sep: TrustSquare is ALREADY an installable app (manifest 200 + all 3 icons 200), so a BOT is one more manifest with a different name, colour and `start_url` — not a new product. Two working prototypes on disk: `genie/bots/collector/BOT_COLLECTOR.html` (find, watch) and `genie/bots/seller/BOT_SELLER.html` (list by talking, coached on the app's OWN `_import_quality_score()`). Voice in and out are free — the phone does both. **PRICING SETTLED (RUL-108): included in Pro at $20, no new price; the selling BOT and watches stay free to every tier.** Two live-site gaps found and worth fixing whether or not BOTs happen: no `apple-mobile-web-app-capable` meta (iPhone icon opens inside Safari with browser bars) and no service worker registered (Android never offers 'Install app'). | Not launch scope. Nothing wired, no flag, nothing in `deploy_manifest.txt`. Revisit after the 20-publisher goal (RUL-096) is settled; the two PWA gaps are separable and improve the existing app on a phone today. | Yes — design and prototypes on disk, nothing shipped |
| 4 Sep | **Genie filter — the search genie as Zoom's front door (RUL-097)** — design AGREED and written up (`ZOOM_HMI_SPEC.md` sec 11; both shapes tappable in `genie/SEARCH_CONCEPT.html`). A floating genie opens a circle of the seven categories with true counts, then hands over to the existing narrowing funnel, which keeps every narrowing decision. David's binding rule: it may never cover another selector — inline band, one full-screen moment only, all controls stay live. Nothing to build until the funnel it fronts is live. | David 4 Sep: *"to be looked at again after we have a stable user base."* STOCK trigger, not a date: (1) Zoom armed in the field (RUL-076, David's act), (2) all seven categories non-zero in a typical city, (3) ~30 days of funnel behaviour to compare against. A ring showing 0 on four of seven advertises an empty shop. | Yes — design is on disk, nothing shipped |
