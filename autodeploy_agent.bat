...
::
::  AGENT-RC-1 (9 Sep 2026): every %var% inside a parenthesised block is expanded when
::  the block is PARSED, not when its line runs. So `set "CLRC=%errorlevel%"` followed by
::  `if "%CLRC%"=="0"` in the same block compared an EMPTY string to "0" on every tick:
::  the CityLauncher deploy completed, was logged "CL FAILED rc= ", the flag was kept, and
::  the same deploy re-ran (and restarted citylauncher.service) every 20 minutes -- 73
::  times between 8 Sep 06:11 and 9 Sep 21:48. The block timestamps were parse-time too.
::  Fix: delayed expansion (!var!) for everything read inside a block. Asserted by RG-0349.
:: ============================================================================
...
call "%~dp0git_unlock.bat" /aged >"%HQ%\git_unlock_aged.txt" 2>&1
