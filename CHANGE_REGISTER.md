...
## 2026-08-01 — INFRA-PANEL-2: Infrastructure card auth-aware loading (stuck "Loading checks…" root-caused)
**Fault:** since INFRA-PANEL-1 shipped (22 Jul), the card showed "Loading checks…" forever whenever the page loaded without a valid admin JWT (fresh session or server-side expiry). Only signal was a small muted "Admin session expired — reload + PIN." note; even after reload+PIN the card stayed stuck up to 5 minutes (next poll). Recurring by construction, not by accident.
**Root cause:** parse-time `infraLoad()` + 401 handler that updated only `#infra-out`, never `#infra-rows`; no reload hook on auth success — the admin gate and the token-gated cards had no signal between them.
**Fix (dashboard.server.html):** `ms-admin-auth` event dispatched from `hideGate()` (all three auth-success paths); infra card listens → instant reload on sign-in. 401 → loud amber "Checks paused" state + Retry. Fetch failure with nothing cached → red "Checks could not load" + Retry. Bootstrap and 5-min poll are token-aware (no doomed calls while signed out). Pattern available to any future token-gated card: listen for `ms-admin-auth`.
**Verify:** node --check on all 10 script blocks; 9-scenario simulated state machine green (no-token boot, poll skip, sign-in reload, mid-session expiry, re-auth recovery, network failure loud). Backup: dashboard.server.html.bak-20260801-infrapanel2. Deploy: next dashboard push (scp dashboard.server.html → server dashboard.html + CF purge); WARNING — deploy_bit_monitoring.bat's server→local pre-pull would overwrite this local fix if run before the push.
**Addendum (same day):** that exact clobber occurred (~09:54, David's bit-monitoring run). Closed at class level: `apply_infra_panel2.py` (idempotent patcher, same survives-the-pull pattern as apply_bit_panel.py / apply_ai_provider_card.py) added as deploy_bit_monitoring.bat step [2c/6] — the fix re-applies on every dashboard deploy. Verified: apply → no-op re-run → node --check 10/10 → simulation 9/9. Live from the next bit-monitoring run.

## 2026-08-10 — SUPER-AFRICA-1: 7-country super-advert expansion, Kenya (Nairobi) pilot — 3-per-category trust ladders
**Scope (David, 10 Aug 2026):** extend supers to NA / MZ / BW / ZW / AO / EG / KE with 3 adverts per category (trust-ladder style, a/b/c tiers), localized city, currency, heritage, culture and REAL current prices. Cities: Windhoek, Maputo, Maun (existing) + Harare, Luanda, Cairo, Nairobi. All 7 as OPEN markets (MAP_NAMING_CANON §7) in waves_policy for the city launch + Wave-1 pre-sent emails. Kenya is the end-to-end PILOT; replication to the other 6 follows the proven path.
**Why CCP:** redefines the super pattern (1-per-category → 3-tier ladder), touches canon (currency/flag/map dicts, ledger city rules, deploy manifest, launch policy) — material per CHANGE_CONTROL_PROTOCOL §1.
**Pilot deliverables (this session, local only — NO deploy):**
- `scripts/seed_super_ladder_global.py` — NEW ladder seeder (reuses seed_super_global clone pattern + supers_ladder_seed persona pattern). KE only for now; COUNTRIES/TIERS extend for the other 6. Photo convention `sup_ke_<catkey>_<tier>_<n>_<name>.jpg` (tier a/b/c). KE must NEVER be added to seed_super_global.py COUNTRIES (its glob would swallow all tiers as one listing).
- `journeys/kenya.json` → build_journey.py → `adventures_ke_map.html` (Leaflet, RG-0011 name, RG-0025 no third-party scripts) + deploy_manifest line.
- ms.js: KE in ADV_COUNTRY_FLAGS / ADV_COUNTRY_CURRENCY (KSh) / ADV_COUNTRY_MAP. regression_ledger.py: nairobi in CITY_CCY/CITY_COUNTRY, KSh in SYMBOLS (RG-0002/0005).
- `KENYA_SUPER_PROMPTS.md` — generated prompt pack for the 24 advert photo sets (~114 photos); journey photos via make_prompt_pack.py as usual. Prices sourced from live Kenyan market research 10 Aug 2026 (Jiji, BuyRentKenya, safari operators; USD/KES ~129).
- waves_policy.json: 7 OPEN-market city entries, gates_green=false, armed=false.
**Known follow-ups (not blocking pilot):** (1) credential signal display names are ZA-anchored (PPRA/NATIS/TGCSA) — needs a localization pass for non-ZA markets (affects US/UK/AU supers too, pre-existing). (2) Tier targets for advexp/advacc/collectors/lm are stored estimates; server-side recompute on first profile view is authoritative (same mechanism as supers_ladder_seed) — diag step at deploy confirms/aligns. (3) Paystack: canon unchanged — introductory service, Tuppence via ZA merchant in ZAR; NOTE Kenya + Egypt are live Paystack merchant countries (KES incl. M-Pesa/Pesalink confirmed 10 Aug 2026) — a future local-rails option, decision deferred.

## 2026-08-10 (later) — SUPER-AFRICA-1 addendum: flight bookends + research canon
- **Kenya map is now a 7-day fly-in safari** (David's ask): Day 1 "Fly in from the Cape" (CPT→NBO, mode:air — existing template convention, zero builder changes) + Day 7 "Fly home" (Mara airstrip→NBO→CPT). 32 stops. Real indicative fare from the live Aviasales Data API (local .secrets token, ZAR native): CPT⇄NBO return from ~R8,900 (10 Aug 2026), shown as cached-indicative with agency-books-it copy — RG-0025 respected (data as build-time text, NO TP script on the page; verified post-build: zero external scripts).
- NEW `SUPER_AFRICA_RESEARCH.md` — advisory levels (Angola Luanda-metro L3 flagged → waves note updated, AO sequencing question), operator landscape (outreach targets only, SO-1), TP rules, fare snapshot, flight-bookend replication pattern. **Open scope question for David: Zambia** (his research includes ZM, not only ZW — Vic Falls straddles both).
- JOURNEY_HIGGSFIELD_PROMPTS.md regenerated: 143 prompts (Kenya 25→32: 7 flight-leg photos added). ms.js KE map blurb updated to fly-in phrasing.

## 2026-08-10 (later still) — SUPER-AFRICA-1: Angola OUT · fly-in bookends replicated · fares cached
- **Angola removed from scope (David):** Luanda deleted from waves_policy (11 cities), research doc row struck. Scope: NA/MZ/BW/ZW/EG/KE (+ Zambia still an open David question).
- **Fly-in bookends replicated to NA/BW/MZ** (photo-preserving: ground days shifted +1, existing photo filenames untouched — verified 23/25/25 still embedded; new f1_/f2_ air-leg stops render as placeholders). Real cached fares on the maps as build-time text: JNB⇄WDH ~R5,200 direct · JNB⇄Maputo ~R3,200 direct · JNB⇄Maun + JNB⇄Nampula thin routes → agency-card fallback copy (doctrine working as designed). RG-0025 verified clean on all three rebuilt maps. ms.js: 3 blurbs to fly-in phrasing + map version bumps (na v10, bw v9, mz v9).
- **FARES_SNAPSHOT_2026-08-10.json** (repo root — NOT in journeys/, the builder/pack glob owns that dir): JNB⇄WDH 5,197 / MPM 3,225 / HRE 3,072 / CAI 7,171 / VNX 9,821 · CPT⇄CAI 12,896 · MUB + APL empty. Feeds ZW/EG bookends at replication.
- Prompt pack 164 (Kenya 32 + 21 flight-leg shots queued); runbook state refreshed. Photo priority: Kenya first (David).

## 2026-08-11 — SUPER-AFRICA-1: map QA fixes + heritage layer + TP links (David's 4 points)
- **Route geometry:** every Kenya day's seg now passes through ALL its stops in order with day-to-day stitching (fixes the off-line Day-6 bush-breakfast pin David flagged, and the same latent class on days 1-3); NA/BW/MZ bookend-day segs rebuilt the same way, Day 7 stitched to Day 6's end — the traveller now visibly returns via airstrip → hub → home airport. Airport pins carry their own photos (KE d7 airstrip/connection/CPT-home are the 3 stills pending).
...
## 2026-08-15 — OPS-SWEEP-1: Report & Fix (David's ask: "make my work less without dropping a ball")
**What:** the always-on half of a Report-&-Fix lane. `scripts/ops_sweep.py` (stdlib, deployed by manifest as `<live>/ops_sweep.py`) runs every 15 min from cron (installed by `migrations/020_ops_sweep_cron.py`) and computes the same verdicts the Ops Map chips show: BEA /health, systemd services (marketsquare/nginx/redis), /health/resources (ram/disk/cpu/bandwidth self-status), fault register (blockers/majors/new queue), SSL days. On any item newly amber/red — or recovered — it emails David one compact report via Resend (key read from the live process environ; from ops@mail.trustsquare.co, reply-to fable@trustsquare.co). Dedupe by state file `.ops_sweep_state.json`; no change = no email.
**Reply commands** (land via the CF catch-all worker → email_triage; the sweep reads them by id watermark): `REPORT` = full report next sweep · `FIX TS-nnnn` = queued in the state file for Fable's morning Cowork pickup · `REVIEW TS-nnnn` = held for David's next session.
**Cockpit:** Report & Fix card added to orchestration_v2/cockpit.html (mailto report-now button + reply commands + honest laptop caveat); cockpit added to the deploy manifest.
